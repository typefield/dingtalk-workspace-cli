...
| "帮我预约一个视频会议" / "开个会" / "发起会议" | 视频会议 | 钉钉客户端（当前 CLI 不支持） | `calendar` | 不要把实时视频会议诉求改写成日历日程 |
| "帮我建一个明天下午的日程" | 日历日程 | `calendar` | — | 日历日程管理（可含参与者/会议室）；视频会议当前 CLI 不支持 |
...
### 4. 视频会议 vs 日历日程 — 视频会议不在当前 CLI

当前 CLI **不提供**视频会议命令。“预约视频会议/开个会/发起会议/会中控制（静音/共享屏幕/录制）/呼叫入会”等诉求，直接告知用户请在钉钉客户端完成，不要试探 `dws conference` 或用其他命令变通。
...
**判断关键**：视频会议相关（发起/预约/控制/邀请）→ 当前 CLI 不支持，请在钉钉客户端操作；日程/参与者/会议室管理 → `calendar`
