#!/usr/bin/env python3
"""Portable result boundary for Multi Todo executable scripts.

The module is kept inside the Skill so an installed Todo Skill does not depend
on another product Skill being present.  It owns flags, one machine writer,
child-process classification, batch channels, and the top-level exception
boundary; product scripts own their business workflow.
"""
...
def run_child_dws(
    ...
    """Do not turn an ambiguous write failure into a safe retry signal."""
    ...
    try:
        ...
        return ChildDWSResult("unknown", error={"type": "network", "message": "dws 调用超时；请求是否已执行未知，请先核查待办状态。"}, command=command)
    ...
    return ChildDWSResult(
        "unknown",
        error={"type": "api", "message": "dws 未返回可解析的终态结果；请求是否已执行未知，请先核查待办状态。", "exit_code": completed.returncode},
        command=command,
    )
    ...
    *, succeeded: Iterable[Mapping[str, Any]] = (), failed: Iterable[Mapping[str, Any]] = (),
    ...
    for channel, entries in channels.items():
        for item in entries:
            ...
            if channel == "unknown" and not isinstance(item.get("reason"), str):
