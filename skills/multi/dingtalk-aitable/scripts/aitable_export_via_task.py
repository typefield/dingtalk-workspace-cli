...
The export task and a local download are distinct operations.  A task that is
still running is reported as ``pending`` with a resumable command; a finished
task is not described as a verified local file until the download has been
atomically committed.  The remote service does not currently provide a
checksum, so local bytes are never advertised as source-integrity verified.
"""
...
def _child_meta(identifier: str, child: ChildDWSResult) -> Optional[dict[str, Any]]:
    return {"id": identifier, "meta": child.meta} if child.meta else None
...
def _download_file(url: str, output_path: Path, *, overwrite: bool) -> tuple[bool, dict[str, Any], dict[str, Any]]:
    """Download into the target directory and publish only with atomic replace."""
    ...
    try:
        fd, temporary_name = tempfile.mkstemp(prefix=f".{output_path.name}.", suffix=".part", dir=output_path.parent)
...
def main() -> int:
    ...
    parser.add_argument("--timeout-ms", type=int, default=1000, help="创建任务的单次等待毫秒数")
    parser.add_argument("--poll-timeout-ms", type=int, default=3000, help="单次任务查询等待毫秒数")
    ...
    parser.add_argument("--dws", default="dws", help=argparse.SUPPRESS)
    ...
    if args.timeout_ms <= 0 or args.poll_timeout_ms <= 0 or args.max_polls < 0:
        return failure(args.format, "--timeout-ms、--poll-timeout-ms 必须为正数，--max-polls 不能为负数。")
    ...
    if args.dry_run:
        return emit(fmt=args.format, outcome="success", data=plan, dry_run=True, text="预览：不会创建导出任务或写本地文件。")

    children: list[dict[str, Any]] = []
    print("[1/2] 创建导出任务", file=sys.stderr)
    started = run_child_dws(_start_args(args), executable=args.dws, timeout=120)
    ...
    start_status = _task_status(task)
    ...
    while not download_url and task_id and polls < args.max_polls:
        ...
        polled = run_child_dws(
            ...
            executable=args.dws,
            timeout=max(120, args.poll_timeout_ms / 1000 + 60),
        )
        if (entry := _child_meta(f"export:poll:{polls}", polled)):
            children.append(entry)
        if polled.state != "success":
            return _pending(
                fmt=args.format, base_id=args.base_id, scope=args.scope, export_format=args.export_format,
                task_id=task_id, file_name=file_name, polls=polls, timeout_ms=args.poll_timeout_ms,
                meta=children,
                last_probe={"state": polled.state, "error": _child_error(polled, "导出任务查询未返回可信结果。")},
            )
    ...
    if not download_url:
        return _pending(
            fmt=args.format, base_id=args.base_id, scope=args.scope, export_format=args.export_format,
            task_id=task_id, file_name=file_name, polls=polls, timeout_ms=args.poll_timeout_ms, meta=children,
        )
    ...
    if args.no_download:
        ...
        return emit(
            fmt=args.format, outcome="success", data=result,
            meta={"children": children} if children else None,
