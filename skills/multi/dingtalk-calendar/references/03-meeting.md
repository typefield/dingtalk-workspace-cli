...
2. **会议室**：用户点名具体会议室 → **禁止**换其他会议室；在用户给定时段内查无该房 → **立刻告知**。**用户未给出时段时，必须先显式向用户追问具体开始/结束时间；禁止默认用「当前时刻至当日 23:59:59」之类窗口代查。** `calendar_schedule_meeting.py` 在收到 `--yes` 前只允许 `--dry-run` 预览；带 `--book-room` 时，先在根目录或用户允许的 `--room-group-id` 内做只读 `room search`，取得可信 `roomId` 后才创建日程、添加参会人并订房。无空房、响应不可投影或查询不可信时，**不创建半成品日程**。写步骤都成功后还会 `event get` 回读；仅 `verification.state=verified` 才可对用户说已确认。**若用户明确限定楼层/楼宇/园区/分组，应先用 `room list-groups` 解析允许的 `group-id`，再把这些 `group-id` 传给脚本 `--room-group-id`（或手工 `room search --group-id ...`）；脚本只会在这些 group 内查找。对于同一地点（同园区/楼栋/楼层）的会议室，必须优先锁定最相关、最贴近该地点的承载 group；该 group 查无 roomId/空房，即可判定该地点当前时段无可订会议室，**不得**再去别的无关 group 继续碰运气。** 在按早停规则查完用户允许范围仍无可用会议室时，必须立即向用户说明结果并收束，禁止继续扩区、换参重试或虚构有房。手工 `room search` **禁止**为试出空闲擅自改日或拉长时间窗。
...
| # | 规范 |
|---|------|
| 1 | **一键脚本**：`calendar_schedule_meeting.py` 做「可选搜房预检 → 建日程 → 加人 → 订房 → 回读」。未限范围时可直接 `--book-room`，脚本只在根目录单次查询；搜房失败/不可信时不创建日程。执行必须有 `--yes --format json`；部分成功保留三通道和 rc=7，回读未证实不伪装为已确认。 |
...
| Recipe             | 行动指南（固定路线） |
| ------------------ | ------------------- |
| schedule-meeting   | **见上文「两准则」**、**「搜房失败硬门禁」**。**未给时段且仅说「发起/开个会」**→ 不走本 recipe；当前 CLI 不支持实时视频会议，告知用户请在钉钉客户端操作。**未给时段但有预约意图**（"安排""约""定"等词）：追问具体开始/结束时间。已有时段后先让用户确认；无明确会议室范围时执行 `python scripts/calendar_schedule_meeting.py --title "<主题>" --start "<起始>" --end "<结束>" [--users <userIds>] [--book-room] --yes --format json`，可先加 `--dry-run` 做零调用预览。脚本在写入前搜房：无可信空房即失败且不创建日程。**有明确范围（某楼/层）**：先 `dws calendar room list-groups`，锁定该地点**最相关的承载 group**；若只有一个地点，`--room-group-id` 应只传这个最相关 group，**不要**把同楼内多个楼层 group 打包传入碰运气。只有用户明确给出多个允许地点时，才把这些 `group-id` 一并传给脚本。**用户点名具体会议室**：须手工 `dws calendar room search --start "<ISO>" --end "<ISO>" --group-id <GROUP_ID> [--available] --format json`（**无** `--query`），在 JSON 中匹配名称取 **`rooms[].roomId` 唯一真值**；不得把用户输入的会议室名当 `roomId`。若搜尽允许范围仍无 roomId/空房，下一条消息必须直接汇报失败；若结果为 `partial_failure` 或 `verification.state!=verified`，先依三通道/回读事实核查，不得重试或宣称已确认。 |
