...
> lite recipe（`list-today-meetings`、`check-users-busy`）见 [lite-recipes.md](./lite-recipes.md)；`start-conference`、`invite-participant`、`share-screen` 见 `dingtalk-conference`。列表类操作须遵循 [calendar.md](./calendar.md) **「CLI 命令树与黄金路径」**，禁止无子命令的 `dws calendar` 或臆造 `calendar list`（见该文 **「反模式（禁止）」**）。预约会议须遵循本文件的时段、会议室与失败收束规则。
...
在用户限定的园区、楼宇、楼层或固定分组内，已按早停规则**逐组 `room search` 查完**仍得不到任何有效 `rooms[].roomId`（或无任何空闲房）→ **立即停止**，向用户**明确报错/失败结论**（例如：该时段在指定范围内未检索到可预订会议室或无法获得 roomId），当前预订链路结束。

**失败处理**：明确说明“指定范围/指定会议室在该时段未找到可预订会议室，因此当前无法完成预订”。只有用户同意放宽地点范围、改时间或接受不订会议室后，才可按新条件继续。

空结果只表示当前授权范围内没有返回合法 `roomId`；不要推测隐藏映射、环境差异或未经用户授权的新搜索范围。
...
出现空结果、`roomId invalid` 或授权范围查尽时，停止当前预订链路；不得沿原假设重复搜索。后续只能报告失败，或在用户确认新范围/新时间后重新搜索。
...
| Recipe             | 行动指南（固定路线） |
| ------------------ | ------------------- |
| schedule-meeting   | **未给时段且仅说“发起/开个会”**时走 lite recipe `start-conference`；有预约意图但缺时段时先确认开始和结束时间。时段明确后固定执行：1. `calendar event create`；2. 有参会人则 `calendar participant add`；3. 再处理会议室。无明确地点范围可用 `calendar_schedule_meeting.py --book-room`；有楼宇/楼层范围时先 `room list-groups` 锁定最相关 group；点名会议室时用 `room search` 返回的 `rooms[].roomId` 调 `room add`。授权范围内无空房则报告不可预订；需要扩大范围或改时间时先取得用户同意。 |
