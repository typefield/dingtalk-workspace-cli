...
| 轻应用 / 快捷应用 / 创建即发布挂工作台 / 轻应用凭证 appKey secret / 轻应用配额软删 | 轻应用全生命周期：创建即发布挂工作台并注册统一应用，无版本流、24h 软删，凭证含 secret 明文 | `dws dev liteapp` | [liteapp.md](references/dev/liteapp.md) |
...
- **Dev JSON 与分页**：所有命令带 `--format json`；`connect list/status` 还必须带它们自己的 `--json`，否则实际输出是表格。标准入口优先使用 `dws dev app ...`；`dws devapp +<shortcut>` 仅作兼容或用户明确指定的入口。`app list`、`permission list`、`event list`、`version list` 只接受游标分页：读取返回的 `meta.pagination.endpoint_exhausted/next_token`，续页传 `--cursor <next_token>`；旧版 `hasMore/nextCursor` 仅作内部映射，不是面向用户的字段。禁止猜 `--page`、`--page-num`，也不要从 `data` 中找旧版字段。
