...
| attendance_schedule_import.py | 排班导入（只读预览 → 用户确认 → 请求受理与终态核查分开表达） | **禁止跳过工作流直接写入**；先读 [attendance-schedule.md](./attendance-schedule.md)，先用 `--dry-run --format json` 取得预览，确认后才传 `--yes` |
...
> - `attendance_schedule_import.py` 由 [attendance-schedule.md](./attendance-schedule.md) 排班导入工作流编排使用，自动处理考勤组校验、班次校验、机器预览、确认门禁及请求/终态边界
