...
查询当前用户自己的可用邮箱地址列表。**仅返回自己的邮箱**，不能查他人邮箱（查他人邮箱请走 [mail-message-commands.md](./mail-message-commands.md) 中「查找他人邮箱地址」三路并发查询流程）。
...
2. 构造 KQL 查询：根据用户意图将搜索条件转为 KQL 表达式（详见 [mail-message-commands.md](./mail-message-commands.md) 中 KQL 查询字段说明）。
...
2. 确定收件人：用户直接提供邮箱地址 → 直接使用；用户提供姓名或工号 → 走「查找他人邮箱地址」三路并发流程（见 [mail-message-commands.md](./mail-message-commands.md)）。
