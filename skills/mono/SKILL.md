...
- 已完成第一阶段迁移的脚本（`todo_batch_create.py`、`aitable_import_via_task.py`、`upload_attachment.py`）可直接使用脚本级 `--format text|json|ndjson` 与 `--dry-run`；其他脚本在各自 Help 明确支持前，不得假定具备这两个参数。
