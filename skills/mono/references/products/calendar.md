...
> **说明**：更新日历信息，最低权限要求：privilege == "owner"。必须至少提供 `--summary` 或 `--desc`；用户主日历本以及他人共享的日历本 **不支持更新**。这是写操作，先 dry-run 核对目标和值，再由用户确认执行。
...
> **说明**：把我的日历共享给指定用户，授予对方相应权限。`--privilege` 可选值：`free_busy_reader`（查看忙闲）、`title_reader`（查看标题）、`reader`（查看详情）、`writer`（创建和编辑）。这是高风险权限授予，必须核对目标用户和权限并由用户确认；`writer` 会允许对方创建和编辑日程。
...
> **说明**：取消我的日历对某人的共享（撤回已授予的访问权限）。aclId 必须先通过 `acl list` 获取并核对目标用户；这是高风险撤权操作，先 dry-run 再由用户确认。
...
| [calendar_schedule_meeting.py](../../scripts/calendar_schedule_meeting.py) | 一键创建日程+添加参会人+预定会议室；先 dry-run 展示计划，确认后以 `--yes` 执行；搜房失败时输出明确原因并返回非零退出码 | `python calendar_schedule_meeting.py --title "复盘会" --start "2026-03-15T14:00" --end "2026-03-15T15:00" --users userId1 --book-room --yes` |
