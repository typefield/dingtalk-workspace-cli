# wiki +resolve-space active Agent 审阅
...
| 真实组织知识库完整目录消歧对拍 | PASS | `rc=0, unified=yes, source_pages=3, candidates=1, count_aligned=yes` |
...
- 普通 `--format json` 现在直接返回唯一的 `ok/outcome/data/meta`；没有协议选择参数或版本字段。

## 边界

当前真实读取证明当前身份可见的组织知识库目录可以在 3 页后观察到 endpoint 耗尽，且一个从目录中选出的名称得到 1 个稳定候选。它不证明个人知识库、权限受限账号、目录超过 100 页、真实重名或服务端异常形状已经出现。
