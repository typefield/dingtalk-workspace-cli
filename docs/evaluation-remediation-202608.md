...
| IM 分页算法与统一返回尚未合流 | **框架基元已落地，两个 IM 命令双验证** | `internal/output.PageLedger` 已实现有限页预算、游标推进/循环校验、耗尽/续页/未知三态、分页 meta 生成，以及首屏失败→`failure`、后续页失败/未知→`partial_failure` 的统一结果映射。只读 `chat +flag-list` 与 `chat +chat-search` 已进入 `dual_validate`：每条命令只执行一次业务流程，同时构建 shadow PageLedger/CommandResult，legacy stdout 与错误行为保持不变；分页矛盾在 shadow 中 fail closed。`chat-search` 的同 cursor 最大窗口探测只计一个逻辑页，探测失败保留首批数据。其余会话、消息和线程回复仍多以各 shortcut 自定义 `complete/hasMore/nextCursor/failures/partial` payload 输出。中央 `server_failure_classifier` 也尚未获得调用的读写/幂等/执行状态。 | 对拍 `+flag-list`、`+chat-search` 的 legacy bytes 与 shadow outcome 后再启用统一返回；随后迁 `+conversation-list`。写调用先接入安全上下文，再治理 retryable |
