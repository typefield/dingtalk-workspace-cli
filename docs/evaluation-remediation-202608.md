...
| Agoal 整域 Runtime Schema exclusion 与 Skill 正向路由矛盾 | **三个只读叶已完成逐命令准入与统一投影；其余 13 条继续审阅** | Mono/Multi Skill 已长期正向教授 `dws agoal`，但 Runtime Schema 曾把 16 条业务叶整体排除，导致 Skill 路由与 Schema 发现面相互矛盾。本轮没有为追求 exclusion=0 批量放开。`agoal user rules` 已补齐 ProductDecl、ContractFinal、read/low/not_required/idempotent Safety 和稳定规则/周期投影，并按独立提交完成 `legacy_only → dual_validate → unified_active`；真实当前用户返回稳定规则，服务端无覆盖证据时明确 `ruleCoverageKnown:false`。第二条 `agoal report list-statistics` 同样逐叶完成 Contract/Safety、参数映射与严格投影：只发布后续 `submit-detail` 需要的 `templateId`、标题、类型、状态、三类提交计数和视图权限，HTML 正文、修改人标识与未审阅配置不进入 Agent 结果；未知字段、重复模板、非整数/矛盾计数和内容形状漂移均返回不可重试的 `projection_unknown`。dual 实跑曾准确拒绝一次“字段缺失等于 null”的错误假设，修正后真实 3 条模板通过 legacy 对拍；active 普通 `--format json` 直接返回 `ok/outcome/data/meta`，`meta.count` 对齐，`reportCoverageKnown:false` 且无伪造分页。第三条 `agoal obj-template list` 只发布稳定模板 ID、标题、类型、状态和权重开关，创建人及完整 dimensions 留在业务边界；服务端 `page/pageSize/totalCount` 与行数必须严格对账，首末页真实读取为 20+15/35，非末页给恢复 token、末页才声明 endpoint exhausted，同时明确 `authoritativeInventory:false` 和 `inventoryCoverageKnown:false`。`agoal report submit-detail` 的真实默认日期读取与统计计数出现口径矛盾，且尚未取得按时/迟交非空完整行，因此记录边界证据后继续保持 exclusion；`agoal user objectives` 当前真实账号也只观察到空数组。证据见 `agent-scans/agoal-user-rules-*-20260810.md`、`agent-scans/agoal-report-statistics-*-20260810.md`、`agent-scans/agoal-obj-template-list-*-20260810.md` 与 `agent-scans/agoal-submit-detail-boundary-20260810.md`。当前全量 Schema 有 3 条 Agoal public tools，其余 13 条保持排除 | 继续按叶审阅读取和覆盖式写入；先为 `user objectives` 取得非空真实投影，为 `report submit-detail` 对齐日期窗口与三种 submitState 的非空行/分页证据。写命令必须补 confirmation、dry-run、旧数据覆盖风险、partial/unknown 与回读证据后才可移出 exclusion。合法 CLI 生命周期、重复 canonical 与敏感数据边界不要求清零 |
