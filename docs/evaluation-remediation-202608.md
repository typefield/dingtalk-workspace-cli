...
| 错误 subtype / 恢复提示缺少闭集治理 | **两批 registry 已落地，机制未关闭** | `scripts/agent/scan_error_contract.py` 从非测试 Go 源码确认 13 个 descriptor、76 个 `WithSubtype` 调用；高频本地校验 `missing_required_flags`、`invalid_flag_value`、`invalid_argument`、`input_read_failed`、`invalid_json_input`、`formula_errors_found` 与下载完整性 `download_output_unavailable`、`download_size_mismatch` 已无自由构造调用，跨产品 `confirmation_required`、`pagination_inconsistent`、`projection_unknown` 同样已治理。Descriptor 现提供可被命令级 hint 覆盖的安全默认恢复提示；真实 CLI 已验证 `invalid_json_input` 返回 `validation/invalid_json_input/rc=3` 和修复 hint。其余仍有 82 个自由字面 `WithReason` 调用、6 个直接 `ErrorInfo.Subtype` 与 16 个动态构造点。`Category`、退出码、`hint/actions/retryable/retry_after_seconds` 已可复用，但未注册值仍不能宣称 wire-stable。扫描只产出 Markdown 评测证据，不保存 JSON fixture，也不把 Agent 审阅替换成 CI。RFC-0003 已明确渐进边界：先治理高频 subtype，不改公开命令/输出选择方式。 | 按 [RFC-0003](rfcs/0003-error-contract-subtype-governance.md) 逐命令迁移高频 subtype 的剩余自由调用，补齐 registry 所需恢复语义；动态上游 reason 必须显式映射或归入 `api/upstream_unclassified`，再逐步要求每个公开 subtype 的 recovery 语义 |
...
| Mono 脚本 Help/Skill 统计与错误路径漂移 | **本轮已关闭（Help/机器错误边界）；7 个深层 dry-run fixture 已取证** | 35 个 Python 文件按 AST 分为 32 个 Agent 入口、3 个内部模块；Agent 扫描实测 32/32 暴露 `--dry-run`、32/32 暴露 `--format`、Help 非零为 0；`attendance_report_checkin.py` 的可选依赖检查已延后到业务执行阶段；全部入口现经 `_runtime.run_main` 进入统一边界：JSON/NDJSON 下未捕获异常返回 `failure + error.type=internal`，不以 traceback 替代结果；`todo_batch_create.py` 的错误类型 `executors` 直接返回 validation；Skill 已说明 `partial_failure`/rc=7 和可选 `meta` 语义 | `scripts/agent/probe_mono_result_contract.py --output docs/agent-scans/mono-result-contract-20260809.md` 实测 6/6：32/32 入口接入、未捕获异常、非零 SystemExit、partial rc=7、meta 与错误类型输入均为单一 JSON stdout；`scripts/agent/probe_mono_dry_run.py --output docs/agent-scans/mono-dry-run-probe-20260808.md` 对 7 个高风险写入口实测 7/7：单一 JSON stdout、无 sentinel dws 调用、无额外本地文件；其余入口的业务参数组合、子 dws meta 实际透传、异常路径和真实账号副作用仍需按台账标记为 `UNVERIFIED` |
