...
| Mail 公开读取列表未知响应被伪装为成功空结果 | **CLI 已 fail-closed；`+thread-list` 已迁入统一返回** | `mail +thread-list`、`+folder-list`、`+tag-list`、`+user-search`、`+template-list`、`+contact-list` 现区分已知空数组与未知响应；未知容器、非法行或无可识别字段返回 typed `projection_unknown`，不再向 Agent 声称“没有会话/文件夹/标签/用户/模板/联系人”。`+thread-list` 还严格投影 `hasMore/nextCursor`：有续页必须带 token、终态不得带普通 token、外层/嵌套矛盾返回 `pagination_inconsistent`；随后才作为该组首条读列表迁入统一返回，`meta.count` 与 `meta.pagination` 不再被丢弃 | 用真实邮箱复验 `+thread-list` 的正常空结果、续页、终态与异常形状；其余读取列表先补各自分页证据，再逐命令纳入统一结果 |
