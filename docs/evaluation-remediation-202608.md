...
| IM 分页不完整错误仍是自由字符串 | **第六批 registry 与首批统一分页读路径已收口** | 群搜索/已加入群/收藏/会话/单聊消息/全量消息/我的群/话题回复的 8 条 `*_incomplete` reason 已改为稳定 subtype；保持既有 reason wire，不改变 legacy 消费方。每条声明为 API + `idempotent_read_only`，命令级 hint 继续要求保留已读结果与 `failures`，按 `nextCursor`/`nextPage` 续读，不能把当前结果当完整。`chat +chat-search`、`+flag-list`、`+conversation-list`、`+thread-replies`、`+chat-messages` 与默认多维检索 `+search-msg` 已实际进入唯一的 `unified_active` 返回，`meta.pagination` 只在 endpoint 证据明确时出现；`+search-msg` 还保留 `indexCoverageKnown:false`，不会把索引空命中扩大成不存在。后续页与资源/导出失败保留成功页并输出 `partial_failure`；[Agent 审阅](agent-scans/im-pagination-rollout-20260809.md) 已更正此前 RFC 的 dual 阶段漂移 | 用真实 IM 账号复验正常多页、空结果、游标冲突、第二页失败及资源下载/导出失败；新增状态机必须按读写安全性、幂等性与终态证据映射稳定 subtype，不能把写请求的模糊失败套用此 read-only 策略 |
...
| IM 分页算法与统一返回尚未合流 | **框架基元已落地；`+flag-list` / `+chat-search` / `+conversation-list` / `+thread-replies` / `+chat-messages` / `+search-msg` 已 active** | `internal/output.PageLedger` 已实现有限页预算、游标推进/循环校验、耗尽/续页/未知三态、分页 meta 生成，以及首屏失败→`failure`、后续页失败/未知→`partial_failure` 的统一结果映射。六条已激活只读命令的普通 `--format json` 直接生成单一 `ok/outcome/data/error/meta` 结果，不公开版本/协议选择参数；本地页上限保留 `endpoint_exhausted:false + next_token`，后续页或明确的分页边界失败保留成功页并返回 `partial_failure`/rc=7。会话列表、消息历史、话题回复与默认多维检索均将 API 的 `hasMore=false + nextCursor=0` 归一为终页；非零、非法或浮点 JSON `MaxInt64` 精度边界的游标都 fail-closed，避免舍入后生成错误续页时间。[chat-message-cursor-boundaries-20260809](agent-scans/chat-message-cursor-boundaries-20260809.md)、[thread-replies-contract-20260809](agent-scans/thread-replies-contract-20260809.md)、[chat-messages-contract-20260809](agent-scans/chat-messages-contract-20260809.md) 与 [im-pagination-rollout-20260809](agent-scans/im-pagination-rollout-20260809.md) 为本地 Agent 扫描证据。`chat +chat-messages` 现已迁入 active：后续读取、分页边界、请求的发送者筛选、显式资源下载与本地原子导出失败均保留成功页并返回 typed `partial_failure`；范围查询已完整时但服务端端点未证实耗尽，会如实省略 pagination meta 并标 `pagination_known:false`，不把业务范围完整扩大为 endpoint exhausted。`chat +search-msg` 现已迁入 active：业务 data 保留 `indexCoverageKnown:false`，但不再泄漏 legacy `contractVersion`/分页字段；未知容器或缺稳定消息 ID 为 `projection_unknown`，显式全量翻页缺证据、后续页、富化或资源下载失败均保留成功页并返回 typed `partial_failure`。标准 `tools/call` runtime 的 MCP/business 错误现统一进入 `server_failure_classifier`：只有 `queryToolMeta` 明确证明业务工具未执行时才保留 `retryable:true`；其余网络不确定失败保留服务端 retry 提示作诊断但不鼓励重放，先核对目标状态。[server-failure-recovery-20260809](agent-scans/server-failure-recovery-20260809.md) 为本地 Agent 扫描证据。 | 以真实账号复验六个 active 入口的可见范围、响应形状和索引健康；其余消息 shortcut 按 legacy→dual→active 逐条迁移。写调用先接入安全上下文，再治理 retryable |
