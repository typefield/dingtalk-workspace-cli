...
| IM 分页算法与统一返回尚未合流 | **框架基元已落地，IM 命令未迁移** | `internal/output.PageLedger` 已实现有限页预算、游标推进/循环校验、耗尽/续页/未知三态、分页 meta 生成，以及首屏失败→`failure`、后续页失败/未知→`partial_failure` 的统一结果映射；框架定向测试已通过。IM 群搜索、会话、收藏、消息和线程回复仍大多以各 shortcut 自定义 `complete/hasMore/nextCursor/failures/partial` payload 输出，部分路径还可同时出现 `paginationKnown:false` 与 `complete:true`。中央 `server_failure_classifier` 也尚未获得调用的读写/幂等/执行状态，不能把上游 `server_retryable` 直接当作写请求的安全重试许可。 | 按 [RFC-0004](rfcs/0004-im-pagination-and-recovery-rollout.md) 先为只读 `chat +flag-list`、`+chat-search`、`+conversation-list` 建 CommandResult adapter；双验证保证 legacy 字节不变后再启用统一返回。写调用先接入安全上下文，再治理 retryable |
