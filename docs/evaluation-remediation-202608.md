...
| 12 条读命令 12 种 JSON 信封 | **框架已接入，渐进迁移中** | Framework 2.0 已提供统一 `ok/outcome/data/error/meta`、强类型布尔、单 writer、partial/pending；不公开协议选择参数，不输出版本标记。`drive download` 与 `sheet export` 已作为文件传输首批迁入；高频只读 `drive +recent`、`+search`、`+search-docs`、`calendar +find-room` 与窄搜索 `+find-file` 现已迁入：已知空列表、可续页和 endpoint 终态映射为 `meta.count/meta.pagination`；三条搜索和会议室可用性查询在服务端未给分页/索引覆盖事实时明确输出 `data.pagination_known:false` / `index_coverage_known:false`，而不把当前搜索页扩大成索引完整。`+find-file` 的未知容器、非对象行或无稳定 `dentryId` 条目，`+search-docs` 的无稳定 `nodeId` 条目，以及 `+find-room` 的无稳定 `roomId` 条目现在 fail-closed；缺列表容器、不可续游标、分页矛盾和无稳定 nodeId/dentryId/spaceId 条目都不再压成空列表或暴露旧 `nextCursor/hasMore` 作为统一结果事实。`scan_rollout_ledger.go` 在隔离配置中从真实 Cobra tree 生成 [Markdown inventory](agent-scans/rollout-ledger-20260809.md)：1,483 个可执行命令节点中 24 条为 dual validation、69 条为 unified active；状态跳级仍明确标为 `REVIEW`，不把同一提交的自对拍误写成历史迁移证据。Generic MCP passthrough dual 阶段现复用 legacy formatter；[Agent 兼容审阅](agent-scans/dual-validate-legacy-compat-20260809.md) 覆盖 `&<>` JSON、原文、空/无 text block 与单次调用 | legacy 命令按 terminal command 继续迁移；后续发布以该 Markdown 作为 `--baseline` 由 Agent 审阅跳级/回退，不得把迁移责任交给 Agent 或接入 CI |
...
| Calendar 公开读取未知响应伪装为空结果 | **CLI 已 fail-closed** | `calendar +agenda`、`+attendee-list`、`+room-search`、`+room-groups`、`+book-list`、`+book-search` 与 `+find-room` 现在区分已知空数组和未知响应；未知容器、非法行或无稳定字段返回 typed `projection_unknown`。`+find-room` 只接受带非空 `roomId` 的会议室条目，并明确分页事实未知，不会把响应结构漂移表述为“没有日程/参会人/会议室/日历本” | 用真实账号分别验证空结果、嵌套容器、各入口的分页与服务端异常形状；endpoint 耗尽语义仍需以实际分页字段为准 |
