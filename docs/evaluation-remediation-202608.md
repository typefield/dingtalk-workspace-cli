...
| `dev connect list` 远端应用名称补全失败被静默吞掉 | **已关闭（当前工作树）** | 本地连接器清单仍以 `success` 返回；远端 `list_dev_app` 失败、未知列表形状、缺/重复续页游标或超过安全页数时，统一信封以可选 `_notice.app_name_enrichment` 明确标记 `unavailable/remote_lookup_failed`，不把“名称未返回”伪装成“应用没有名称”。专项回归覆盖成功补全、离线补全和不完整分页 | 用真实 devapp 账号复验名称解析、分页和仅本地连接器场景；本地健康状态不依赖远端名称查询 |
...
| IM 分页算法与统一返回尚未合流 | **框架基元已落地；`+flag-list` / `+chat-search` / `+conversation-list` / `+thread-replies` / `+chat-messages` 已 active** | `internal/output.PageLedger` 已实现有限页预算、游标推进/循环校验、耗尽/续页/未知三态、分页 meta 生成，以及首屏失败→`failure`、后续页失败/未知→`partial_failure` 的统一结果映射。五条已激活只读命令的普通 `--format json` 直接生成单一 `ok/outcome/data/error/meta` 结果，不公开版本/协议选择参数；本地页上限保留 `endpoint_exhausted:false + next_token`，后续页或明确的分页边界失败保留成功页并返回 `partial_failure`/rc=7。会话列表、消息历史与话题回复均将 API 的 `hasMore=false + nextCursor=0` 归一为终页；非零、非法或浮点 JSON `MaxInt64` 精度边界的游标都 fail-closed，避免舍入后生成错误续页时间。[chat-message-cursor-boundaries-20260809](agent-scans/chat-message-cursor-boundaries-20260809.md)、[thread-replies-contract-20260809](agent-scans/thread-replies-contract-20260809.md) 与 [chat-messages-contract-20260809](agent-scans/chat-messages-contract-20260809.md) 为本地 Agent 扫描证据。`chat +chat-messages` 现已迁入 active：后续读取、分页边界、请求的发送者筛选、显式资源下载与本地原子导出失败均保留成功页并返回 typed `partial_failure`；范围查询已完整时但服务端端点未证实耗尽，会如实省略 pagination meta 并标 `pagination_known:false`，不把业务范围完整扩大为 endpoint exhausted。标准 `tools/call` runtime 的 MCP/business 错误现统一进入 `server_failure_classifier`：只有 `queryToolMeta` 明确证明业务工具未执行时才保留 `retryable:true`；其余网络不确定失败保留服务端 retry 提示作诊断但不鼓励重放，先核对目标状态。[server-failure-recovery-20260809](agent-scans/server-failure-recovery-20260809.md) 为本地 Agent 扫描证据。 | 以真实账号复验五个 active 入口的可见范围、响应形状和索引健康；其余消息 shortcut 按 legacy→dual→active 逐条迁移。写调用先接入安全上下文，再治理 retryable |
