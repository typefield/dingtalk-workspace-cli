...
| devapp shortcut 列表投影/分页丢失 | **已通过 Agent 审阅并按命令迁入统一结果；真实环境待复验** | `+list`、`+permission-list`、`+event-list`、`+version-list` 只把明确空数组投影为成功空列表；缺列表容器、容器非数组、非法行或缺 `unifiedAppId`/`scopeValue`/`eventCode`/`versionId` 的展示行均 fail-closed 为不可重试的 `api/projection_unknown`，不会把响应漂移说成“没有资源”。四条随后收集顶层及嵌套 `result/data/content/pageInfo/pagination` 的 `hasMore/nextCursor`，再投影到分页 meta；字段类型错误、`hasMore=true` 缺游标、末页带游标以及跨层事实冲突均为不可重试的 `validation`（`pagination_invalid/incomplete/conflict`）。每条在原 CLI 路径使用普通 `--format json` 直接产生统一结果；端到端回归锁定单次调用、无版本标记、投影列表，以及 `meta.pagination.endpoint_exhausted:false/next_token`。Agent 审阅证据：`docs/agent-scans/devapp-pagination-projection-20260809.md` | 用真实 devapp 账号验证已知空、正常项、续翻、末页和异常字段；同步对拍 `dev` 原子命令与 `devapp` shortcut |
...
| Drive 列表/搜索未知响应伪装为空结果 | **CLI 已 fail-closed；目录列表进入 dual validate** | `drive +list`、`+search`、`+find-file`、`+search-docs` 现在区分已知空结果与无法投影的响应；未知容器、非法行或缺少可识别字段返回 typed `projection_unknown`。`+list`、`+search` 与 `+find-file` 的非空文件条目必须带稳定 `dentryId`，`+search-docs` 条目必须带 `nodeId`，空间搜索条目必须带 `spaceId`；不再把只有展示名、无法继续执行的对象交给 Agent，也不把后端形状漂移扩大成“无文件/无文档”。`+list` 本轮仅从 legacy 进入 `dual_validate`：外部 payload 保持不变，内部候选结果校验 `meta.count`、一致分页与 `inventory_scope=requested_location`，明确不把单个 space/folder 页扩大成全部钉盘目录 | 真实目录、空目录、搜索无命中和服务端异常形状分别复验；完成脱敏 live 分页/范围证据后才考虑单命令晋级 unified active |
...
| Todo 聚合 shortcut 将未知响应伪装为空集合；后续页错误丢弃已读结果 | **读取候选已进入 dual_validate，待真实环境复验** | `+related-tasks`、`+due-today`、`+created-todos`、`+overdue` 与写前定位的 `+todo-done` 复用严格 `todoCards` 投影：仅明确数组可继续；缺容器、容器非数组、非法行或无稳定 `taskId` 均在本地 fail-closed 为不可重试的 `api/projection_unknown` / `response_projection`。本轮将 `+related-tasks`、`+due-today` 从 legacy 迁入 `dual_validate`：同一次聚合读取会保留第二页失败前的 cards，未来 active 候选以 `partial_failure` / rc=7 保留投影后的成功页和 typed failed page；dual 阶段仍输出原 legacy bytes / 原错误，不要求 Agent 改调用或解析。Todo API 未提供权威 continuation，候选一律声明 `data.pagination_known:false`，不把短页扩大为 endpoint exhausted。`+todo-done` 继续通过 fail-closed facade 读取，任何不完整聚合都会丢弃已读 cards 并在写请求前返回错误。Agent 审阅证据：`docs/agent-scans/todo-aggregate-projection-20260809.md` | 用隔离账号复验正常空列表、多页、第二页失败、权限受限和未知形状；`+todo-done` 再验证不完整聚合时绝不发写请求；完成服务端边界证据后逐命令评审是否晋级 active |
...
| Chat 已加入群列表未知响应伪装为空结果 | **CLI 已 fail-closed；两个公开群目录入口均进入 dual_validate，并完成真实单页投影复验** | `chat +chat-list-all` 与 Agent 主入口 `chat +my-groups` 现在仅对明确识别的群数组返回空列表。未知容器、非法群条目或无稳定 `conversationId` 返回 typed `projection_unknown`；自动翻页中后续页异常则保留已读群及 `failures` 台账，标记 `stopReason=projection_error` 且不安全重试，避免将部分群目录当完整结果。当前源码构建二进制对 `+my-groups --limit 1` 的真实只读扫描返回 1 条投影群、`count` 对齐、稳定 `conversationId` 与可判定的续页事实，见 `agent-scans/chat-my-groups-live-projection-20260809.md`。本轮 `+my-groups` 与 `+chat-list-all` 均从 legacy 进入 dual_validate：外部仍保持历史 JSON，内部由 `PageLedger` 候选结果表达端点耗尽、可续页、未知边界及后续页 `partial_failure`；`+chat-list-all` 还拒绝没有稳定 `openConversationId` 的展示行，证据见 `agent-scans/chat-list-all-dual-validation-20260809.md`。不公开版本或 rollout 参数。 | 用真实账号验证空群、跨页、游标错误和网关响应形状漂移；真实群目录覆盖范围仍不由 CLI 擅自承诺，完成脱敏 live evidence 后才按单命令评审两个入口进入 active |
...
| AITable/Base 目录死条目与假阴 | **CLI 已 fail-closed，服务端未关闭** | 列表/搜索结果显式标记发现来源、分页已知性和索引覆盖未知，不再承诺权威全量目录；`query_records` 完整结果使用 `endpointExhausted:true`，不再输出语义过宽的 `complete` 键；`base list/search` 会聚合顶层与 `result/data` 的分页证据：`hasMore=true` 无游标、`hasMore=false` 带游标、字段非布尔、多个 continuation 不一致或外层/嵌套 `hasMore` 矛盾都会返回不可重试的 `pagination_inconsistent`，一致的嵌套证据才会投影分页事实，不再产出不可续翻的成功结果。`+base-list/+base-search` 本轮只从 legacy 进入 `dual_validate`：公共 JSON 仍保持历史 payload，框架在进程内严格校验 count、分页和稳定的 string `baseId`，因此还未让 Agent 切换路由或解析方式。当前真实最近访问读取保留 `authoritativeInventory:false` / `paginationKnown:false`，一条投影记录的 `baseId` 与 `count` 对齐，见 `agent-scans/aitable-base-discovery-live-20260809.md`；它不能证明目录精度或全量覆盖。 | 后端治理死条目；真实账号复测精度和召回率；完成 dual evidence 后才逐命令晋级 unified active |
...
| Mono 脚本 Help/Skill 统计与错误路径漂移 | **Help/机器错误边界与关键写编排已收口；真实终态仍待验证** | 35 个 Python 文件按 AST 分为 32 个 Agent 入口、3 个内部模块；Agent 扫描实测 32/32 暴露 `--dry-run` 与 `--format`，Help 非零为 0。所有入口均经 `_runtime.run_main` 统一机器异常边界：未捕获异常、stdout 污染、非法 JSON、无 typed error、meta/dry_run 类型错误或结果/退出码矛盾，都会改写为单一 typed failure；旧 `success:false`、非布尔/矛盾状态和子 pending 都不会冒充终态成功。Skill 已说明 `partial_failure`/rc=7、三通道和 meta。 | `probe_mono_result_contract.py` 为 **24/24 PASS**，覆盖 32/32 入口异常边界、严格 child 结果解析与代表性写编排；`probe_mono_dry_run.py` 为 **10/10 PASS**，覆盖文档、OA、Todo、字段、记录、AITable 文件导入/导出、日程、附件与邮件的受控 dry-run：单一 JSON stdout、无 sentinel dws 调用、无额外本地文件。两者均是临时 child-runner 证据，真实账号副作用、异常参数组合及服务端终态仍为 `UNVERIFIED`。 |
| Mono 邮件/日程/表格写入结果真实性 | **代码已收口，真实终态待验证** | 邮件旧 `success:false` 不再误报已发送；日程在事件创建后参会人/订房失败时保留 `succeeded[]/failed[]/unknown[]` 并返回 `partial_failure`。记录导入、字段创建、AITable 文件导入和附件上传均保留确认成功、明确失败与未知写入事实。异步导入在后续 `import_data` 不可信时保留 importId、阶段及 per-step meta；异步导出则将任务与本地文件分开：创建/轮询不可信不建议重试，任务 pending 有 next_command，本地下载以原子落盘和 SHA-256 表达，且无上游 checksum 时明确不是远端验证。 | `mono-result-contract-20260809.md` 为 **24/24 PASS**；`mono-aitable-export-contract-20260809.md` 为 **10/10 PASS**，仅以临时 child runner/HTTP server 对拍脚本编排、任务结果表达与本地原子写入。仍须在隔离账号验证邮件、日程、导入/导出、字段和附件的真实最终状态；受控 fixture 不等同于服务端 exactly-once 证据。 |
...
- **2026-08-09 复扫**：`mono-script-contract-20260809.md` 确认 35 个 Python 文件中 32 个为 Agent 入口，Help 为 `32/32` dry-run、`32/32` format、0 个非零；`mono-result-contract-20260809.md` 对共享 `run_main`、stdout 污染拒绝、结果/退出码一致性、异常/非零 `SystemExit`、partial rc=7、meta 类型、严格 child 状态和关键写编排为 `24/24 PASS`。记录导入、字段创建、文件导入和附件上传都保留确认成功、明确失败或不确定状态，文件导入还保留两步 child meta。`mono-dry-run-probe-20260809.md` 以临时 HOME/工作区和 sentinel dws 对 10 个深层门控写入口验证单一 JSON stdout、无子进程调用、无额外本地文件；`mono-aitable-export-contract-20260809.md` 另以临时 child runner/HTTP server 对异步导出任务、pending、失败、本地原子下载与覆盖保护为 `10/10 PASS`。这些证据均不替代真实后端零写、账号权限或服务端终态验证。
