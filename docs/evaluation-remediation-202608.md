...
| 错误 subtype / 恢复提示缺少闭集治理 | **八批 registry + 文档/event/IM/统一输出/复合写 family 已落地，机制未关闭** | `scripts/agent/scan_error_contract.py` 从非测试 Go 源码确认 63 个 descriptor、118 个直接 `WithSubtype` 调用和 11 个经测试的有限间接映射；高频本地校验、输入/公式/下载完整性、自然目标解析、AITable 目标预检、版本预检、transport/服务端响应、本地 flag/Skill 市场、文档复合写固定 reason family、`event_stop_unverified`、IM 八条幂等只读 `*_incomplete` 分页 family、unified output 的 `pagination_*` / `invalid_success_type` / `skill_setup_*`，以及批量聊天写、文档授权和文档分享的三条部分写失败 family 均已进入稳定 registry。第八批保持 legacy `reason` wire，并把每条写失败显式约束为不可盲重放：通过 hint/actions 要求先检查 succeeded/failures、当前权限或 recipients 台账，仅处理未确认对象。原始 `errorCode` 与本地原因只保留在诊断字段。未知 HTTP/RPC/响应体和网络观测继续归 category 对应的 `upstream_unclassified` / `discovery_upstream_unclassified`，保留 `http_status`、`rpc_code`、trace、`transport_failure` 等诊断事实但不再把上游数字或网络文本拼成 Agent subtype；401/403、限流、协议不兼容和后端依赖不可用也有独立恢复边界。`tools/call` 的模糊 408/5xx/网络失败仍不宣称安全重试。doc create/checkpoint-update/history-revert 与 event stop 已在 dual validation 构建严格 outcome result，同时保留 legacy error/rc，尚未转 active。其余仍有 43 个自由字面 `WithReason` 调用、9 个直接 `ErrorInfo.Subtype`（现已全部登记）与 1 个动态构造点，13 个 subtype 缺有效恢复提示；enum 转 string 写法由 Agent 扫描纳入统计。扫描只产出 Markdown 评测证据，不保存 JSON fixture，也不把 Agent 审阅替换成 CI。 | 按 [RFC-0003](rfcs/0003-error-contract-subtype-governance.md) 逐命令迁移剩余自由 subtype，补齐 registry 所需恢复语义；剩余个人订阅状态机必须连同 Category、幂等性与终态语义单独设计，不能仅作字符串替换；文档/event 的 typed result bridge 按 [RFC-0005](rfcs/0005-operation-outcome-bridge.md) 逐命令完成 legacy golden 与受控真实账号验证后再转 active，再逐步要求每个公开 subtype 的 recovery 语义 |
| IM 分页不完整错误仍是自由字符串 | **第六批 registry 已收口读路径** | 群搜索/已加入群/收藏/会话/单聊消息/全量消息/我的群/话题回复的 8 条 `*_incomplete` reason 已改为稳定 subtype；保持既有 reason wire，不改变 legacy 消费方。每条声明为 API + `idempotent_read_only`，命令级 hint 继续要求保留已读结果与 `failures`，按 `nextCursor`/`nextPage` 续读，不能把当前结果当完整。`go test ./internal/errors ./internal/shortcut/chat ./internal/shortcut/smart` 通过 | 继续将其余 43 个自由 reason 按读写安全性、幂等性与终态证据逐条审阅；不能把写请求的模糊失败套用此 read-only 策略 |
