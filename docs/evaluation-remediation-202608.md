...
| devapp credentials shortcut 缺少敏感输出声明 | **代码已修** | `devapp +credentials-get` 已补齐 Identity、Safety、ResultSpec 和 `SensitivePaths`，与 `dev app credentials get` 的敏感字段声明对齐；rollout 测试锁定该 Contract | 发布后二进制确认 Schema 不把 secret 写入示例、日志或普通选型说明 |
