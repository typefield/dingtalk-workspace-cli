...
| devapp shortcut 列表分页投影丢失 | **代码已修，待端到端复验** | `+list`、`+permission-list`、`+event-list`、`+version-list` 先收集顶层及嵌套 `result/data/content/pageInfo/pagination` 的 `hasMore/nextCursor`，再投影到分页 meta；字段类型错误、`hasMore=true` 缺游标、末页带游标以及跨层事实冲突均 fail-closed 为不可重试的 `validation`（`pagination_invalid/incomplete/conflict`），不会再静默丢弃证据并输出成功列表。Agent 审阅证据：`docs/agent-scans/devapp-pagination-projection-20260809.md` | 用真实 devapp 账号验证续翻、末页和矛盾分页字段；同步对拍 `dev` 原子命令与 `devapp` shortcut |
