...
| 错误 subtype / 恢复提示缺少闭集治理 | **十二批 registry + 文档/event/IM/统一输出/复合写/插件协议/认证发现/路由校验 family 已落地，机制未关闭** | `scripts/agent/scan_error_contract.py` 从非测试 Go 源码确认 **88** 个 descriptor、**149** 个直接 `WithSubtype` 调用和 **11** 个经测试的有限间接映射。第十二批将命令歧义、未知 shortcut/子命令、重复参数、批量 PAT 确认、ID 交集和不支持文档类型纳入稳定 registry：它们都是请求发出前的本地 validation，保留既有 reason、rc 与 Help 行为，并给 Agent 稳定的恢复分支，不制造新的重试路径。第十一批的认证刷新、网关鉴权过期、未登录/未配置、Raw API 凭证缺失、endpoint 未解析与文档下载预检也持续保持安全登录、配置与发现动作；Raw API 仍不把 MCP 默认凭证伪装成可用凭证。未知 HTTP/RPC/响应体和网络观测继续归 unclassified subtype，`tools/call` 的模糊 408/5xx/网络失败仍不宣称安全重试。doc create/checkpoint-update/history-revert 与 event stop 已在 dual validation 构建严格 outcome result，同时保留 legacy error/rc，尚未转 active。其余仍有 **13** 个自由字面 `WithReason` 调用、9 个直接 `ErrorInfo.Subtype`（均已登记）与 1 个动态构造点；扫描到的 subtype **0** 个缺有效恢复提示。扫描只产出 Markdown 评测证据，不保存 JSON fixture，也不把 Agent 审阅替换成 CI。 | 按 [RFC-0003](rfcs/0003-error-contract-subtype-governance.md) 逐命令迁移剩余自由 subtype，补齐 registry 所需恢复语义；剩余个人订阅状态机必须连同 Category、幂等性与终态语义单独设计，不能仅作字符串替换；文档/event 的 typed result bridge 按 [RFC-0005](rfcs/0005-operation-outcome-bridge.md) 逐命令完成 legacy golden 与受控真实账号验证后再转 active，再逐步要求每个公开 subtype 的 recovery 语义 |
| IM 分页不完整错误仍是自由字符串 | **第六批 registry 已收口读路径** | 群搜索/已加入群/收藏/会话/单聊消息/全量消息/我的群/话题回复的 8 条 `*_incomplete` reason 已改为稳定 subtype；保持既有 reason wire，不改变 legacy 消费方。每条声明为 API + `idempotent_read_only`，命令级 hint 继续要求保留已读结果与 `failures`，按 `nextCursor`/`nextPage` 续读，不能把当前结果当完整。`go test ./internal/errors ./internal/shortcut/chat ./internal/shortcut/smart` 通过 | 继续将其余 **13** 个自由 reason 按读写安全性、幂等性与终态证据逐条审阅；不能把写请求的模糊失败套用此 read-only 策略 |
| 统一输出直接 subtype 未登记/缺恢复提示 | **第七批已关闭直接分支键缺口** | DevApp 分页 `pagination_invalid` / `pagination_incomplete` / `pagination_conflict`、shortcut 非布尔 `success`、以及 Skill 安装全失败/非法 partial 均改用登记 enum，并输出 hint；DevApp 分页错误还带 `operation=devapp.pagination_projection`，shortcut 类型漂移带 `operation=shortcut.response_projection`，Skill 安装全失败带 `operation=skill.setup`。现有结果类型、reason、退出码不变；Agent 扫描确认 9 个直接 `ErrorInfo.Subtype` 均已登记 | 当前全仓还剩 **13** 个 legacy `WithReason` 与 1 个动态构造点，但扫描到的 subtype 已无缺失恢复提示；不得把“已登记”误写成真实服务端终态已验证 |
