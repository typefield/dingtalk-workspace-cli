...
| IM 分页算法与统一返回尚未合流 | **框架基元已落地；`+flag-list` / `+chat-search` / `+conversation-list` / `+thread-replies` 已 active，`+chat-messages` 双验证** | `internal/output.PageLedger` 已实现有限页预算、游标推进/循环校验、耗尽/续页/未知三态、分页 meta 生成，以及首屏失败→`failure`、后续页失败/未知→`partial_failure` 的统一结果映射。四条已激活只读命令的普通 `--format json` 直接生成单一 `ok/outcome/data/error/meta` 结果，不公开版本/协议选择参数；本地页上限保留 `endpoint_exhausted:false + next_token`，后续页或明确的分页边界失败保留成功页并返回 `partial_failure`/rc=7。会话列表、消息历史与话题回复均将 API 的 `hasMore=false + nextCursor=0` 归一为终页；非零、非法或浮点 JSON `MaxInt64` 精度边界的游标都 fail-closed，避免舍入后生成错误续页时间。[chat-message-cursor-boundaries-20260809](agent-scans/chat-message-cursor-boundaries-20260809.md) 与 [thread-replies-contract-20260809](agent-scans/thread-replies-contract-20260809.md) 为本地 Agent 扫描证据。`chat +thread-replies` 现已迁入 active：缺失 `hasMore`、`hasMore=true` 空页、续页矛盾、后续读取和显式资源下载失败都保留成功页，不伪称全量成功。`chat +chat-messages` 仍为 `dual_validate`；其资源下载与本地原子导出失败均已写入 shadow `partial_failure`，但 legacy stdout/错误保持不变。其余消息仍多以各 shortcut 自定义的 legacy 分页 payload 输出。中央 `server_failure_classifier` 也尚未获得调用的读写/幂等/执行状态。 | 以真实账号复验四个 active 入口的可见范围、响应形状和索引健康；完成 `+chat-messages` 的 legacy bytes、shadow 与 active outcome 对拍后再逐条迁移。写调用先接入安全上下文，再治理 retryable |
