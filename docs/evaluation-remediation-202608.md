...
| IM 分页算法与统一返回尚未合流 | **框架基元已落地；`+flag-list` / `+chat-search` / `+conversation-list` 已 active，余两条双验证** | `internal/output.PageLedger` 已实现有限页预算、游标推进/循环校验、耗尽/续页/未知三态、分页 meta 生成，以及首屏失败→`failure`、后续页失败/未知→`partial_failure` 的统一结果映射。三个只读命令的普通 `--format json` 直接生成单一 `ok/outcome/data/error/meta` 结果，不公开版本/协议选择参数；本地页上限保留 `endpoint_exhausted:false + next_token`，未知边界省略分页断言，后续页失败保留成功页并返回 `partial_failure`/rc=7。会话列表和消息历史均将 API 的 `hasMore=false + nextCursor=0` 归一为终页，而非误报分页矛盾；非零或非法终页游标仍 fail-closed。消息历史与话题回复还拒绝浮点 JSON 在 `MaxInt64` 的精度边界，避免舍入后生成错误续页时间；[chat-message-cursor-boundaries-20260809](agent-scans/chat-message-cursor-boundaries-20260809.md) 为本地 Agent 扫描证据。Agent 扫描 [chat-flag-list-contract-20260809](agent-scans/chat-flag-list-contract-20260809.md)、[chat-search-contract-20260809](agent-scans/chat-search-contract-20260809.md) 与 [chat-conversation-list-contract-20260809](agent-scans/chat-conversation-list-contract-20260809.md) 均仅验证本地 fixture 契约，不替代真实账号。`chat +thread-replies` 与 `chat +chat-messages` 仍为 `dual_validate`：每条仅执行一次业务流程，同时构建 shadow PageLedger/CommandResult，legacy stdout 与错误行为保持不变。两条命令现已把显式 `--download-resources` 的局部下载失败写入 shadow `partial_failure`，保留成功读取/下载和逐项失败详情；`chat-messages --output` 也已将“消息读取成功、原子本地导出失败”构建为 shadow `partial_failure`：激活后会保留成功页，失败项带 `operation=chat/message_export`、本地路径和覆盖意图；在 dual 阶段仍返回原始导出错误并保持 legacy stdout 不变。其余消息仍多以各 shortcut 自定义的 legacy 分页 payload 输出。中央 `server_failure_classifier` 也尚未获得调用的读写/幂等/执行状态。 | 以真实账号复验三个 active 入口的收藏/群搜索/会话召回、响应形状和索引健康；完成 legacy bytes、shadow 与 active outcome 对拍后，逐条迁移余两个只读入口。写调用先接入安全上下文，再治理 retryable |
...
| Mono 脚本 Help/Skill 统计与错误路径漂移 | **Help/机器错误边界与关键写编排已收口；真实终态仍待验证** | 35 个 Python 文件按 AST 分为 32 个 Agent 入口、3 个内部模块；Agent 扫描实测 32/32 暴露 `--dry-run`、32/32 暴露 `--format`、Help 非零为 0；`attendance_report_checkin.py` 的可选依赖检查已延后到业务执行阶段；全部入口现经 `_runtime.run_main` 进入统一边界：JSON/NDJSON 下未捕获异常返回 `failure + error.type=internal`，不以 traceback 替代结果。运行时还验证缓存后的机器 stdout：JSON 必须是一条 `ok/outcome` 信封且与 0/1/7 退出码一致，NDJSON 每个非空行必须是对象；遗留 `print()`、多行、非法 JSON 或结果/退出码矛盾均被拒绝并转为单一 typed failure，原始污染文本不会回显给 Agent。共享 `run_child_dws` 对统一 `ok:false`、旧信封布尔 `success:false` 和顶层非布尔 `ok`/`success` 都不视为成功；后者以 `untyped_status` 标为 `unknown`，不可证实的写入终态同样保守保留为 `unknown`。`todo_batch_create.py` 的错误类型 `executors` 直接返回 validation；待办、审批、文档、记录导入、字段创建、文件导入与附件上传均保留明确成功、执行前失败和终态未知，审批任务解析失败不会发送占位 task ID，文档 update 不会自动重放；`mail_send_with_cc.py` 已接入同一边界，旧业务失败不会被误报为已发送。Skill 已说明 `partial_failure`/rc=7、三通道和可选 `meta` 语义 | `scripts/agent/probe_mono_result_contract.py --output docs/agent-scans/mono-result-contract-20260809.md` 实测 **19/19**：32/32 入口接入、未捕获异常、stdout 污染拒绝、结果/退出码一致性、子 dws 严格布尔/非布尔状态识别、非零 SystemExit、partial rc=7、meta、错误类型输入、邮件旧业务失败与关键受控写入编排均为单一 JSON stdout；`scripts/agent/probe_mono_dry_run.py --output docs/agent-scans/mono-dry-run-probe-20260809.md` 对 8 个高风险写入口实测 8/8：单一 JSON stdout、无 sentinel dws 调用、无额外本地文件；其余入口的业务参数组合、其他脚本的子 dws meta、异常路径和真实账号副作用仍需标记为 `UNVERIFIED` |
| Mono 邮件/日程/表格写入结果真实性 | **代码已收口，真实终态待验证** | `mail_send_with_cc.py` 不再把旧 `success:false` 业务失败报告为“已发送”；`calendar_schedule_meeting.py` 在事件已创建、参会人或订房后续失败/不确定时保留 `succeeded[]`，并将后续步骤放入 `failed[]/unknown[]`，输出 `partial_failure`、rc=7；缺少 eventId 同样不伪装成功。`import_records.py` 按批次保存确认成功、明确失败和终态未知，后续批次不再抹掉前序成功；`bulk_add_fields.py` 不再把旧业务 `success:false` 报告为“字段创建完成”；`aitable_import_via_task.py` 在上传成功后若 `import_data` 未给出终态，会保留 `importId`、阶段与 `execution_state:unknown`，不建议盲目重复触发；`upload_attachment.py` 则在 PUT 未得终态时保留 `fileToken`、上传阶段和未知状态，不误报附件可用。`mono-result-contract-20260809.md` 现为 **19/19 PASS**，覆盖旧业务失败、非布尔状态、日程与表格写入后续失败及共享信封边界 | 用隔离账号验证邮件、事件、参会人、会议室、导入记录、字段创建、文件导入与附件上传的真实最终状态；受控 child-runner 不等同于服务端 exactly-once 证据 |
...
- **2026-08-09 复扫**：`mono-script-contract-20260809.md` 重新确认 35 个 Python 文件中 32 个为 Agent 入口，Help 为 `32/32` dry-run、`32/32` format、0 个非零；`mono-result-contract-20260809.md` 对共享 `run_main` 的未捕获异常、stdout 污染拒绝、结果/退出码一致性、非零 `SystemExit`、partial rc=7、meta 透传、严格布尔/非布尔子状态识别、邮件旧业务失败和关键 child-dws 编排路径均为 `19/19 PASS`。因此“类型错误导致 traceback 打破 JSON”“脚本信封不能承载 meta”以及“旧 `success:false` 被当作邮件已发送”都是修复前快照的问题；记录导入不再把已确认批次压成总布尔失败，字段创建不再将旧业务失败误报为成功，文件导入在 `import_data` 未给终态时会保留任务标识，附件上传在 PUT 未得终态时也不再误报为可用。`mono-dry-run-probe-20260809.md` 用临时 HOME/工作区和 sentinel dws 验证 8 个深层门控写入口均未调用子进程、未新增本地文件；它仍不替代真实后端零写或其余 24 个入口的逐路径验证。
