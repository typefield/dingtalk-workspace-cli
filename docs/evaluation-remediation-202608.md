...
| 错误 subtype / 恢复提示缺少闭集治理 | **十五批 registry 已关闭变量 reason；复合写已 active，真实服务端终态仍待审阅** | `scripts/agent/scan_error_contract.py` 从非测试 Go 源码确认 **105** 个 descriptor、**208** 个直接 `WithSubtype(...)` 或兼容桥调用和 **11** 个有限间接映射；字面与变量 `WithReason` 已从 31 降为 **0**，9 个直接 `ErrorInfo.Subtype` 均已登记，所有 stable subtype 均有有效恢复提示。扫描表现在区分显式 `retryable:true`、`false`、未声明和未知表达式，避免把 `WithRetryable(false)` 误读为“可重试”；第十五批个人订阅、toolbar `SYSTEM_BUSY`、`request_build_failed` 与模糊 `tools/call` 失败的恢复边界维持原有保守语义。Doc create/checkpoint-update/history-revert 与 event stop 已按 terminal command 完成 dual→active；成功、preview、partial 和 failure 均走单一结果 lifecycle，其中 Doc 三条命令的 active data 固定为 `operation/result/steps`，不会嵌套旧信封。Agent 扫描见 `agent-scans/doc-operation-outcomes-20260810.md`。扫描只产出 Markdown 评测证据，不保存 JSON fixture，也不把 Agent 审阅替换成 CI。 | 用受控账号验证个人订阅服务端对 idempotency key 的去重、响应丢失和历史 cooldown 语义；真实验证 Doc 创建后 JSONML 失败、checkpoint 更新失败、回滚读回失败及补偿事实；不得把 registry、本地 fixture 或 active 输出扩大为服务端终态已经验证 |
...
| 复合写失败没有稳定恢复指令 | **源码错误分类已收口；服务端终态仍待验证** | 首批 `batch_write_failed`、`doc_grant_permission_partial_failure` 与 `doc_share_message_failed` 已登记为 API + `RetryNever`，并保留逐项 details、hint 与 actions。当前全量 Agent 扫描已覆盖 105 个 descriptor、208 个直接稳定 subtype 调用点和 11 个有限间接映射：自由字面/变量 `WithReason` 均为 0，直接 `ErrorInfo.Subtype` 未登记为 0，缺有效 hint 的 subtype 为 0，无法解析 Category 的 subtype 为 0。旧“46→43 / 16→13”只代表早期批次，不能继续作为当前基线 | 继续审阅 11 个有限间接映射及真实恢复动作；用隔离账号验证“上游报错但实际已写”的终态，不能把 registry 标注等同于服务端 exactly-once 证明 |
...
| devapp 核心写 shortcut 将 API success 扩大为业务终态 | **五条核心命令已逐条迁入统一结果；删除二次确认已同源** | `devapp +create/+update/+enable/+disable/+delete` 与对应 `dev app` 原子入口共用 `DevAppMutationResultSpec` 和成功投影。普通 `--format json` 的成功 data 显式带 `verification.state=not_verified`，说明当前没有写后回读；非对象响应 fail-closed 为不可重试的 `api/projection_unknown`，并标记 `execution_started:true`，提示先查询目标状态、禁止盲目重试。`+update` 零变更字段在业务调用前返回 typed validation。`+delete` 新增 guard-first、`--confirm-name` 和同源应用名提取：dry-run 零调用；真实执行在用户确认后只读 `get`，名称缺失/不匹配均 fail-closed，精确匹配后才调用一次 delete。首批四条 [结果扫描](agent-scans/devapp-core-write-outcomes-20260810.md) 为 6/6 PASS，删除 [安全扫描](agent-scans/devapp-delete-safety-20260810.md) 为 5/5 PASS；[rollout ledger](agent-scans/rollout-ledger-devapp-delete-20260810.md) 记录唯一的 `dual_validate → unified_active` 合法单步迁移 | 用隔离应用完成真实创建、修改、启停和删除后的回读终态验证；特别验证删除响应丢失、已删除时 get 的确定性 not-found，以及重复请求不会被 Agent 盲目重放。当前受控 caller 不证明服务端 exactly-once |
| devapp 成员批量写把请求列表扩大为逐成员成功 | **两入口结果与 Safety 已同源；Shortcut 保持 dual 等待真实响应证据** | `dev app member add/remove` 与 `devapp +member-add/+member-remove` 共用 `DevAppMutationResultSpec` 和 mapper。添加统一为 write/high，移除统一为 destructive/high，均 user_required/idempotency unknown。成功对象保留上游响应，同时把 `unifiedAppId/memberType/userIds/count` 放入 `data.requested`，并声明 `verification.state=not_verified`；`requested` 明确不是逐成员成功通道。非对象响应或请求身份丢失 fail-closed 为 `projection_unknown`，标记 execution_started，禁止盲目重试。四条公开 dry-run 证明零写：native 直接统一信封，既有 Shortcut 逐字保留 legacy JSON。Agent 扫描见 [devapp-member-mutation-contract-20260810.md](agent-scans/devapp-member-mutation-contract-20260810.md)，7/7 PASS | 官方仓库和现有评测没有 helper 的脱敏真实成功响应；取得对象形态证据并用 `member-list` 对 requested userIds/role 回读前，两条 Shortcut 不晋级 active，不宣称 per-user terminal success |
| devapp 版本写把模糊 ACK / 审批中扩大为发布完成 | **结果、Safety 与恢复状态已收口；真实发布终态未关闭** | `dev app version create/check-approval/publish/status` 与 `devapp +version-*` 现共用四类 ResultSpec 和同一结果 mapper。创建必须返回稳定 `versionId`；成功只保留 `requested` 并标记 `verification.state=not_verified`。审批人选择、`approvalSubmitted`、`AUDIT/UNDER_REVIEW` 输出 `pending + meta.operation.next_command`；发布只有明确 `published=true` 或 `versionStatus=RELEASE/GRAY` 才能进入 success，且仍要求 status/get 回读。模糊对象 ACK、`published=false` 且没有可恢复 operation、请求/响应 ID 冲突均 fail-closed 为不可重试的 `projection_unknown`。状态分类会遍历 completion/status/task/version/process/approval，`FAIL/PUBLISH_FAILED/WITHDRAW/CANCEL` 优先于笼统 SUCCESS/AUDIT。create/publish 两入口均 write/high、user_required、idempotency unknown、guard-first。Agent 扫描见 [devapp-version-lifecycle-contract-20260810.md](agent-scans/devapp-version-lifecycle-contract-20260810.md)，8/8 PASS | 当前没有隔离应用的真实创建/发布、审批链和回读证据；两个旧写 Shortcut 继续 `dual_validate` 并保留 exclusion，不因受控 runner 通过而公开或晋级。真实验证需覆盖直接 RELEASE、SELECT_APPROVER、企业审批、拒绝/撤回、响应丢失和重复请求 |
...
| `wiki +node-list` 挂起/续页失败 | **统一投影与真实续页已修；空目录仍待 fixture** | 已发布 canonical `wiki.shortcut_node_list` 及 read/low/not_required/idempotent Safety，业务只调用一次 `doc/list_nodes`。旧投影虽识别 `hasMore`，却漏了真实服务字段 `nextPageToken`：实测 `--limit 1` 可稳定复现“有续页但被 CLI 拒绝为 `pagination_inconsistent`”。当前已将 `nextPageToken/next_page_token` 纳入不透明游标，移除重复的顶层 scope 扫描，并对每行强制非空 string `nodeId`；非对象行、缺 ID、ID/展示字段类型漂移现整页 fail-closed，不再静默丢行后返回 success。显式 `--limit` 也收口为 1-50，非法值在零远端调用前返回 `validation/invalid_flag_value`、rc=3。源码构建二进制的真实只读对拍现确认：首页 count=1、`endpoint_exhausted:false`且 token 可用；传回 token 后第二页仍有稳定 ID，与首页不重复。旧的 46 条根目录终页证据仍保留。脱敏证据见 `agent-scans/wiki-node-list-live-projection-20260809.md` 与 `agent-scans/wiki-node-list-pagination-live-20260810.md` | 有界检查 20/33 个可读文件夹未找到空目录，因此空目录仍精确记为未验证，不构造资源；嵌套分页矛盾继续由本地 response-seam 覆盖，遇到真实嵌套样本再补 live 证据 |
| Doc 搜索/节点列表未知响应伪装为空结果 | **真实分页与严格投影已修；空文件夹仍待 fixture** | `doc +search` 与 `doc +list` 仅接受明确列表、对象行和非空 string `nodeId`；名称、类型、URL、创建人、时间、`hasChildren` 与 workspace 等展示/导航字段也按真实类型投影，类型漂移整页 fail-closed 为 typed `projection_unknown`。两条命令现严格读取服务端真实 `hasMore + nextPageToken`，一致时写入 `meta.pagination`，缺 token、终页带 token、非布尔状态、非安全数字游标或上下层矛盾均返回不可重试 `pagination_inconsistent`。搜索始终保留 `index_coverage_known:false`，不会把 endpoint 耗尽扩大为索引健康；列表固定 `inventory_scope=requested_location`，不会扩大为递归或租户全量目录。显式 limit 与搜索时间范围在远端调用前校验。源码构建二进制的脱敏只读 Agent 对拍已确认 list/search 首页和第二页 token 可恢复且 ID 不重复、随机无命中仅声明 endpoint 终页、非法 limit 为 rc=3；见 `agent-scans/doc-discovery-pagination-live-20260810.md` | 有界扫描未取到真实空文件夹，因此空目录仍精确记为 REVIEW；继续用真实权限受限与异常形状复验，本地 response seam 不替代服务端样本 |
| `drive list --pattern` 失效 | **代码已修，真实只读正反例通过** | `drive list` 已公开 `--pattern`；当前回归覆盖当前页通配过滤、JSON 可解析、保留服务端 `nextToken`、未知响应 fail-closed，以及与 `--versions` 的冲突。源码临时构建二进制又对真实根页执行通配、从内存派生的精确名称和确定无命中三组测试，结果分别为 41/41、1/1、0；5 项小页过滤前后 continuation 保持一致。Agent 证据仅保存计数与布尔结论，见 `agent-scans/drive-pattern-live-20260809.md`。客户端过滤只声明当前 endpoint 页，绝不把未读取的后续页伪装成目录已耗尽 | 继续用真实子目录、权限受限目录和续页做复验；当前根页通过不证明整个 Drive 目录覆盖或服务端召回率 |
...
| AITable/Base 目录死条目与假阴 | **CLI 投影、解析与统一输出已收口，服务端精度/召回率未关闭** | `+base-list/+base-search` 已从 `dual_validate` 单步晋级 `unified_active`；Agent 使用 `--format json` 直接获得顶层 `ok/outcome/data/meta`，不再解析 historical 裸 payload。列表/搜索结果显式标记 `sourceKind`、`authoritativeInventory:false`、`inventoryCoverageKnown:false`，搜索额外标记 `indexCoverageKnown:false`；空命中不扩大成业务上不存在。顶层与 `result/data` 的分页证据会聚合校验：`hasMore=true` 无游标、`hasMore=false` 带游标、字段非布尔、多 continuation 不一致或外层/嵌套矛盾均返回不可重试的 `pagination_inconsistent`。续页真相现只出现在 `meta.pagination`，业务 `data` 仅保留 `paginationKnown`，不再重复输出 `hasMore/endpointExhausted/nextCursor/complete`。真实只读 Agent 对拍于内存中从最近访问结果取名称做搜索：两条命令均返回 1 条稳定 `baseId`、count/meta 对齐且 stderr 为空，未保存名称、ID、查询词或原始 JSON，见 `agent-scans/aitable-base-discovery-live-20260809.md`。`+resolve-base` 进一步移除独立的容器、通用 `id/name/title` 和递归分页猜测器：只接受稳定 `baseId/baseName`，通过 `PageLedger` 穷尽连贯页面后才允许唯一精确/显式 fuzzy 选择；空结果的 typed error 明确 `coverage_scope=name_search_index/index_coverage_known=false`。本轮真实只读解析恰好复现服务端 cursor 停滞，active 命令返回不可重试的 `pagination_inconsistent` 且未发布候选，见 `agent-scans/aitable-resolve-base-unified-projection-20260810.md`。`+base-get` 也已按独立提交完成 `legacy_only -> dual_validate -> unified_active`：只发布请求 ID 一致的 `baseId/baseName`、稳定 `tableId/dashboardId` 和计数；重复 ID、字段漂移及未取证的非空 `documents` 返回不可重试的 `projection_unknown`，并以 `inventoryCoverageKnown:false` 拒绝把资源目录扩大成权威清单。真实只读 dual/active 对拍均通过且未保存 Base 身份或原始 JSON，见 `agent-scans/aitable-base-get-dual-validation-20260810.md`、`agent-scans/aitable-base-get-unified-projection-20260810.md`。Skill 正向路由已统一到 `+base-list/+base-search/+resolve-base/+base-get`；rollout ledger 记录逐命令合法迁移。 | 后端继续治理死条目、搜索索引和本轮真实 cursor 停滞；用真实账号复测已删除 Base、漏召回 Base、空目录、正常多页耗尽和权限受限。当前正常单页发现、单个 Base 目录与 fail-closed 解析证据都不证明服务端目录精度、资源覆盖或索引覆盖 |
| `aitable +list-tables` 猜测容器并把未知/非法表目录当成功 | **严格表目录投影与渐进迁移已完成，真实只读通过** | 旧实现会猜测 `tables/list/items/records/data/result`，静默丢弃非对象条目，缺少 ID/名称仍输出 null；完全找不到列表时还会把未知原始响应按 success 返回。当前只接受真实审阅的 `success -> data.tables[] -> tableId/tableName` 链路；未知字段/容器、非对象条目、通用 `id/name` 别名、空或重复稳定 ID、缺名称及错误的 fields/views 类型均返回不可重试的 typed `projection_unknown`。命令以独立提交完成 `legacy_only -> dual_validate -> unified_active`，普通 `--format json` 直接输出 `ok/outcome/data/meta`，结果固定为 `data.tables[] + meta.count`，无协议选择参数或版本字段。脱敏真实 Agent 对拍从内存选择一个 Base，将原子 `table get` 与 shortcut 的稳定 ID/名称集合对拍，双方均为 1 条且完全一致；证据见 `agent-scans/aitable-list-tables-dual-validation-20260810.md`、`agent-scans/aitable-list-tables-unified-projection-20260810.md`。 | 继续验证真实空 Base、多表 Base、权限受限、新增服务字段和上游异常；当前一条真实记录不证明其他 Base 可见范围，`get_tables` 无分页只依据已审阅 endpoint 契约，不扩大为企业全部 Base 目录 |
| `aitable +resolve-table` 与表目录维护独立猜测投影 | **共享严格消歧与渐进迁移已完成，真实只读通过** | 旧 resolver 独立递归猜测 `result/data/list/items/tables/records`，接受通用 `id/name/title` 并静默丢弃非法行，容易与 `+list-tables` 对同一响应得出不同事实。当前两入口复用唯一严格表目录投影；只在完整 `get_tables` 目录中匹配，默认必须大小写不敏感精确命中，包含匹配仅在显式 `--fuzzy` 时启用，零/多候选返回 typed failure 而不猜选。命令以独立提交完成 `legacy_only -> dual_validate -> unified_active`；active 数据固定为 `resolved/matchType/count/candidates`，普通 `--format json` 无协议版本字段。脱敏真实 Agent 从原子 `table get` 在内存中选择表名，resolver 返回 exact 且稳定 ID/名称完全一致；证据见 `agent-scans/aitable-resolve-table-dual-validation-20260810.md`、`agent-scans/aitable-resolve-table-unified-projection-20260810.md`。 | 继续验证真实重名、显式 fuzzy 多候选、空 Base、权限受限和上游新增字段；当前单候选只证明该 Base 本次可投影，不证明所有 Base 或跨账号可见性 |
...
| `contact +org` 终结 passthrough 无法兼顾 legacy 与审阅投影 | **框架扩展与命令渐进迁移已完成，真实只读通过** | Shortcut 新增命令级内部 `ResultMapper`：dual 阶段对同一次 MCP 响应构造/校验统一结果，但仍交给 legacy writer 原字节输出；active 阶段只输出映射后的 `ok/outcome/data`，不增加协议 flag 或版本标记。`+org` 以两个独立提交完成 `legacy_only -> dual_validate -> unified_active`；成功数据只含 `deptId/deptName/memberCount`，稳定 `deptId` 必需，未知容器/缺 ID fail-closed 为 `projection_unknown`。真实 Agent 探针在内存中用当前身份姓名读取部门，确认 3 个审阅字段、稳定 deptId、stderr 为空且业务 exactly-once；证据见 `agent-scans/contact-org-dual-validation-20260809.md`、`agent-scans/contact-org-unified-projection-20260810.md` | 继续验证重名消歧、无部门、权限受限和字段漂移；当前身份成功不证明全组织目录或其他账号可见性 |
| `contact +team` raw 成员 passthrough 与 canonical 投影分叉 | **共享投影与渐进迁移已完成，真实只读通过** | `+team` 与 canonical `+list-dept-members` 现共用唯一成员投影，统一识别 `deptUserList/userInfo` 并要求每条稳定 `userId`；未知容器、非法行或缺 ID 会使整个结果 fail-closed，不再输出成功空/残缺列表。命令按 `legacy_only -> dual_validate -> unified_active` 两个提交迁移：dual 保持 legacy writer 与 exactly-once，active 只输出 `data.count/members + meta.count`。真实当前部门读取为 19 条，三处 count 对齐且全部有稳定 userId；证据不保存成员资料，见 `agent-scans/contact-team-dual-validation-20260810.md`、`agent-scans/contact-team-unified-projection-20260810.md` | 继续验证重名消歧、空部门、下级部门边界、权限受限和字段漂移；当前本部门读取不证明全组织覆盖 |
| `contact +lookup` 完整资料 raw 包装缺少稳定结果边界 | **完整资料投影与渐进迁移已完成，真实只读通过** | 该命令没有复用 `+me` 的精简投影，而是为完整资料定义独立 ResultSpec：身份、组织、部门、职位、标签和联系方式全部进入审阅 Schema，5 条敏感路径显式声明；唯一用户、稳定 userId 及三类集合逐项稳定 ID 是成功前提。命令以独立提交完成 `legacy_only -> dual_validate -> unified_active`，dual 阶段保持原 legacy bytes 和 exactly-once，active 只输出统一 `ok/outcome/data`。真实当前用户读取确认 8 个可用顶层字段、1 个部门、1 个职位、空标签集合和稳定 ID；上游合法的可选 `orgId:null` 被省略，非空非法 ID、未知字段、类型漂移、别名冲突或多用户响应继续 fail-closed。脱敏证据见 `agent-scans/contact-lookup-dual-validation-20260810.md`、`agent-scans/contact-lookup-unified-projection-20260810.md` | 继续验证重名消歧、其他用户、非空标签、权限受限和新增字段；当前用户成功不证明目录覆盖或跨租户可见性 |
| `contact +resolve-dept` 未知搜索响应会被压成零命中 | **严格消歧投影与渐进迁移已完成，真实只读通过** | 旧实现会遍历多个猜测容器并静默丢弃非对象候选，缺 ID/名称也会以空字符串进入结果；任意 float ID 还会被截断。当前只接受真实审阅的 `success/deptList/hasMore/totalCount` 形状，每条必须有稳定整数 deptId 和可消歧名称，非法行、未知字段、重复/分数/零 ID 均 fail-closed。`hasMore:true`、缺分页事实或终页 total 不对账返回 typed `pagination_inconsistent`，不再用当前页判断唯一性；根部门 `-1` 归一为下游可用的 `1`。命令以独立提交完成 `legacy_only -> dual_validate -> unified_active`；active 数据固定为 `resolved/count/candidates`，分页终态只在观察到 endpoint 耗尽时进入 `meta.pagination`。真实读取为两个稳定候选，count/meta 对齐；证据见 `agent-scans/contact-resolve-dept-dual-validation-20260810.md`、`agent-scans/contact-resolve-dept-unified-projection-20260810.md` | 继续验证零/唯一命中、真实根部门、权限受限和分页异常；当前多候选终页不证明其他边界 |
| `wiki +resolve-space` 使用错误 ID 字段并把搜索首屏当唯一结果 | **完整目录消歧与渐进迁移已完成，真实只读通过** | 真实 `search_wikiSpaces` 候选使用 `workspaceId`，旧 resolver 只识别 `spaceId/space_id/id`，因此曾实测输出 `resolved:true` 但 `spaceId` 为空；同时搜索响应没有分页/覆盖事实，单条命中不能证明全局唯一。当前 resolver 改为以 `pageSize=50` 逐页读取 `list_wikiSpaces`，使用共享 PageLedger 验证 `hasMore/nextPageToken`，只有观察到当前身份可访问的组织知识库 endpoint 耗尽后才按名称匹配。候选只接受真实 `workspaceId/name`，未知容器、非法行、空/重复 ID、游标缺失/矛盾/循环及 100 页上限均 fail-closed。命令以独立提交完成 `legacy_only -> dual_validate -> unified_active`；active 数据固定为 `resolved/count/candidates`，普通 `--format json` 直接返回统一结果且无版本字段。脱敏真实 Agent 对拍独立耗尽 3 页目录，resolver 返回 1 个稳定候选并与期望集合一致；证据见 `agent-scans/wiki-resolve-space-dual-validation-20260810.md`、`agent-scans/wiki-resolve-space-unified-projection-20260810.md` | 继续验证个人知识库、权限受限账号、真实重名、超过 100 页和服务端异常；当前 3 页/单候选成功不证明这些边界 |
...
| Agoal 整域 Runtime Schema exclusion 与 Skill 正向路由矛盾 | **四个只读叶已完成逐命令准入与统一投影；其余 12 条继续审阅** | Mono/Multi Skill 已长期正向教授 `dws agoal`，但 Runtime Schema 曾把 16 条业务叶整体排除，导致 Skill 路由与 Schema 发现面相互矛盾。本轮没有为追求 exclusion=0 批量放开。`agoal user rules` 已补齐稳定规则/周期投影，服务端无覆盖证据时明确 `ruleCoverageKnown:false`；`agoal report list-statistics` 只发布后续详情所需模板 ID、规则摘要和严格对账的提交计数，正文 HTML、修改人及未审阅配置不进入 Agent 结果；`agoal obj-template list` 只发布稳定模板摘要，真实首末页为 20+15/35，非末页给恢复 token、末页才声明 endpoint exhausted，并明确不是企业权威模板目录。第四条 `agoal contract fields` 真实返回 11 条字段定义，field ID/code 全部非空且唯一；统一投影只发布 ID、code、标题、类别、类型和四个激活/必填布尔值，presentation-only `scheme` 与当前恒空 `source` 只校验不发布，未知字段、重复 ID/code、字符串布尔和布局形状漂移均 fail-closed，同时以 `fieldCoverageKnown:false` 避免把无分页证据的响应扩大为权威目录。四条命令均按独立提交完成 `legacy_only → dual_validate → unified_active`，普通 `--format json` 只返回 `ok/outcome/data/meta`，不含版本选择参数或标记。`agoal report submit-detail` 的真实默认日期读取与统计计数出现口径矛盾且未取得三种状态的非空完整行；`agoal user objectives` 当前规则的 18 个已知周期仍只观察到空数组；`strategy list` 与 `contract list` 的个人、根部门和 10 个直属部门有界抽样各 12 次也全部为空，未取得非空业务行，三条命令均继续保持精确 exclusion。两条 scope list 另补本地枚举边界：`DEPT/PERSONAL` 大小写归一，其他 `--scope-type` 在任何业务调用前返回 typed `validation/invalid_flag_value`、rc=3。证据见 `agent-scans/agoal-user-rules-*-20260810.md`、`agent-scans/agoal-report-statistics-*-20260810.md`、`agent-scans/agoal-obj-template-list-*-20260810.md`、`agent-scans/agoal-contract-fields-*-20260810.md`、`agent-scans/agoal-strategy-contract-list-boundary-20260810.md` 与 `agent-scans/agoal-submit-detail-boundary-20260810.md`。当前全量 Schema 有 4 条 Agoal public tools，其余 12 条保持排除 | 继续按叶审阅读取和覆盖式写入；先为 `user objectives`、`strategy list`、`contract list` 取得非空真实投影，为 `report submit-detail` 对齐日期窗口与三种 submitState 的非空行/分页证据。写命令必须补 confirmation、dry-run、旧数据覆盖风险、partial/unknown 与回读证据后才可移出 exclusion。合法 CLI 生命周期、重复 canonical 与敏感数据边界不要求清零 |
| Agoal `user objectives` / `report submit-detail` 输入与跨 endpoint 口径 | **本地输入已 fail-closed；业务投影继续 exclusion** | `--period-ids` 现要求至少一个非空 ID；`--submit-state` 只接受 `ON_TIME/LATE/NOT_SUBMITTED` 并归一大小写；非法 ISO-8601 `--query-date`、非正整数 `--page/--page-size` 都在业务调用前返回 typed `validation/invalid_flag_value`、rc=3，无远端请求。Agent 真实扫描建立用户、规则、18 个周期和模板前置事实；目标读取仍为 18 个周期/0 行，明确日期的提交详情为 `ON_TIME=0/LATE=0/NOT_SUBMITTED=1`，但同一模板的 `list-statistics` 计数为 `0/0/0`。两个 endpoint 口径不一致时不用一侧覆盖另一侧，不把空结果扩大为组织内不存在数据。本次仅观察到 `NOT_SUBMITTED` 非空行的 `publishTime/reportId/user` 及用户四个字段，不足以定义三状态统一 ResultSpec。脱敏证据见 `agent-scans/agoal-objectives-submit-boundary-20260810.md`，扫描结果为 `REVIEW` 而非 `PASS` | 保持两条命令精确 exclusion；取得 objectives 非空行、三种 submitState 稳定字段与分页证据，并由后端解释统计/详情的日期或权限口径后，再进入 dual validation |
| Agoal `scorecard detail` 服务端 `null` 被当成成功 | **空成功已修；完整实体契约继续审阅** | 真实根部门返回非 null 计分卡外壳，但核心实体数组为空；直属部门有界抽样 10 个时，旧实现有 8 个响应被原样输出为 JSON `null`、stderr 为空且 rc=0，Agent 无法区分“没有计分卡”和“服务端状态未知”。当前命令对 JSON `null` 返回不可重试的 typed `api/projection_unknown`、非零退出且 stdout 为空；非 null 响应仍交给原 legacy writer，业务 exactly-once，没有借修复空成功改变兼容输出。真实复验为 `null_rejected=8`、`empty_success=2`、`rc0_null=0`，证据见 `agent-scans/agoal-scorecard-detail-boundary-20260810.md` | 抽样未观察到非空核心 `content`，因此命令继续保持精确 exclusion；取得非空 entityId、嵌套目标和 update 所需完整实体样本后再定义 ResultSpec 并进入 dual validation |
| D10 per-Skill 版本声明 | **已关闭（Agent 可验证）** | 14/14 个 mono/multi Skill 根文件均声明独立 SemVer `version`，并保留独立的 `cli_version: ">=1.0.15"`；各 Skill 已按自身路由/结果语义变更独立递增，不要求所有包保持同一个版本。`scripts/agent/scan_skill_cli_versions.py` 同时校验字段存在性、SemVer 格式和 CLI 兼容版本覆盖，当前 PASS | 后续 Skill 内容/路由/安全契约发生不兼容变化时递增对应 Skill 版本；不得只改 CLI 版本或所有版本静默保持不变 |
...
| public leaf 依赖 Schema exclusion | **兼容 helper 已清零；产品边界 exclusion 已逐条审阅** | `contact label list/get/list-members`、`todo task list-sub/remove-attachment`、Chat/DING/OA/Calendar 的已审阅命令均已完成 Identity、接口/参数、Safety 和 Agent 选型审阅并移出 exclusion；当前 Runtime shortcut 为 415 条，其中 383 条进入 public，32 条保留隐藏，32/32 已有明确 Agent 决策理由。保留项集中在敏感 HR 数据、应用/订阅/权限生命周期写操作、重复旧入口以及分页/结果契约尚不足的读取入口；不因可执行就强行公开。当前源码两次确定性装配均为 31 个产品、1127 个运行时 Schema 工具，registry/source hash 一致 | 继续按真实产品证据决定是否开放；保留隐藏是产品边界，不再作为无人审阅债务 |
...
1. 真实环境继续复验 contact 投影的空/权限受限边界、`wiki +node-list` 空文件夹、event stop、todo participant 与 approval create-instance；Wiki 真实续页已完成对拍，不再列为泛化的“待复验”。
2. 在隔离数据上完成 sheet 二次回滚官方自证。
3. 继续复验 DevApp 权限受限、其他账号和异常响应；对五条 active 核心写命令完成真实
   写后回读，确认 `verification:not_verified` 不被消费方扩大成终态成功；删除还需验证
   响应丢失和重复调用边界。终页位置 cursor 不得
   输出为 `next_token`，当前结构计数不得扩大成企业应用目录或权限覆盖完整。
