...
- 当前 `skills/mono/scripts/` 中 34 个可执行 Python 脚本逐个执行 `python3 <script> --help`；
  第二阶段迁移后 23 个 Help 声明脚本级 `--dry-run`，18 个声明脚本级 `--format`，5 个脚本的
...
- 本轮已先迁移 `todo_batch_create.py`、`aitable_import_via_task.py`、
  ...
  `todo_overdue_check.py`、`minutes_recent_summary.py`、`minutes_extract_todos.py`：十八者均接受 `--format text|json|ndjson` 和
  `--dry-run`；
