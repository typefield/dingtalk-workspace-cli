...
- 当前 `skills/mono/scripts/` 中 34 个可执行 Python 脚本逐个执行 `python3 <script> --help`；
  第二阶段迁移后 23 个 Help 声明脚本级 `--dry-run`，12 个声明脚本级 `--format`，5 个脚本的
...
- 本轮已先迁移 `todo_batch_create.py`、`aitable_import_via_task.py`、
  ...
  `calendar_schedule_meeting.py`、`mail_send_with_cc.py`、`oa_pending_review.py`：十二者均接受 `--format text|json|ndjson` 和
  `--dry-run`；
