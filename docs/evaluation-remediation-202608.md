...
| 12 条读命令 12 种 JSON 信封 | **框架已接入，渐进迁移中** | Framework 2.0 已提供统一 `ok/outcome/data/error/meta`、强类型布尔、单 writer、partial/pending；不公开协议选择参数，不输出版本标记。`drive download` 与 `sheet export` 已作为文件传输首批迁入；高频只读 `drive +recent` 与 `+search` 现已迁入：已知空列表、可续页和 endpoint 终态映射为 `meta.count/meta.pagination`；`+search` 在服务端未给分页事实时明确输出 `data.pagination_known:false`，而不把当前搜索页扩大成索引完整。缺列表容器、不可续游标、分页矛盾和无稳定 nodeId/dentryId/spaceId 条目都 fail-closed，不再压成空列表或暴露旧 `nextCursor/hasMore` 作为统一结果事实。`scan_rollout_ledger.go` 在隔离配置中从真实 Cobra tree 生成 [Markdown inventory](agent-scans/rollout-ledger-20260809.md)：1,483 个可执行命令节点中 24 条为 dual validation、66 条为 unified active；状态跳级仍明确标为 `REVIEW`，不把同一提交的自对拍误写成历史迁移证据。Generic MCP passthrough dual 阶段现复用 legacy formatter；[Agent 兼容审阅](agent-scans/dual-validate-legacy-compat-20260809.md) 覆盖 `&<>` JSON、原文、空/无 text block 与单次调用 | legacy 命令按 terminal command 继续迁移；后续发布以该 Markdown 作为 `--baseline` 由 Agent 审阅跳级/回退，不得把迁移责任交给 Agent 或接入 CI |
...
| Drive 列表/搜索未知响应伪装为空结果 | **CLI 已 fail-closed** | `drive +list`、`+search`、`+search-docs` 现在区分已知空结果与无法投影的响应；未知容器、非法行或缺少可识别字段返回 typed `projection_unknown`。`+list` 与 `+search` 的非空文件条目还必须带稳定 `dentryId`，空间搜索条目必须带 `spaceId`；不再把只有展示名、无法继续执行的对象交给 Agent，也不把后端形状漂移扩大成“无文件/无文档” | 真实目录、空目录、搜索无命中和服务端异常形状分别复验 |
