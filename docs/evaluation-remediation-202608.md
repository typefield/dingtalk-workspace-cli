...
| 全域影子 shortcut 扫描 | **未关闭** | 当前 Runtime Schema 实测 415 条 shortcut，其中 379 条 public、36 条 exclusion；4 条 exclusion 已 review，仍有 32 条必须逐条决定公开/删除/保留。已新增 `scripts/agent/scan_shortcut_exclusions.py` 生成逐条 Markdown 队列，避免用一个总数掩盖审阅缺口；本轮将已有 Contract/Safety 的 `minutes +minutes-search`、`calendar +find-room`、`oa +pending`、`oa +done-approvals` 移入 public，并同步 Runtime 目录与 Mono/Multi Skill。attendance 已完成，但真实测试跟进清单仍有 156 条；其中包含真实后端报错，也包含尚未完成稳定接口/安全审阅的命令，不能一键公开 | 按产品逐条 Agent 审阅；写命令先审 Safety/dry-run，再决定公开或删除；在逐条 Contract/Safety/结果投影证据齐全前，不把 exclusion 强行清零 |
