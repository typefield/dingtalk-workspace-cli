...
| IM 分页算法与统一返回尚未合流 | **RFC 已立项，代码未迁移** | IM 群搜索、会话、收藏、消息和线程回复已有游标去重、页数上限、满页无游标探测与 fail-closed 投影，但大多仍以各 shortcut 自定义 `complete/hasMore/nextCursor/failures/partial` payload 输出，未将 authoritative pagination 放入统一 `meta.pagination`。部分路径还可同时出现 `paginationKnown:false` 与 `complete:true`；后续页失败先写 legacy payload 再返回普通错误，不能直接视为 Framework `partial_failure`。中央 `server_failure_classifier` 也尚未获得调用的读写/幂等/执行状态，不能把上游 `server_retryable` 直接当作写请求的安全重试许可。 | 按 [RFC-0004](rfcs/0004-im-pagination-and-recovery-rollout.md) 先为只读 `chat +flag-list`、`+chat-search`、`+conversation-list` 建 PageLedger 和 CommandResult adapter；双验证保证 legacy 字节不变后再启用统一返回。写调用先接入安全上下文，再治理 retryable |
