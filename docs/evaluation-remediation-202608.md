...
| Multi 脚本 Help/写入预览与结果漂移 | **dry-run 已取证；文件导入首批收口，其他脚本仍为 legacy** | 52 个 Python 文件、42 个 Agent 入口；逐个 Agent 执行 `--help` 为 42/42。AITable import/field/record/attachment 四个明确写入口支持脚本级 `--dry-run`，受控 fixture 不调用 dws/OSS。本轮将新建表文件导入 `aitable_import_via_task.py` 迁入 Multi 自有 `_runtime.py`：`--format text|json|ndjson`、`--dry-run`、单一 JSON 结果、异常兜底与 child DWS 的明确失败/终态未知分类均由同一边界处理；导入触发不可信时保留 `importId/phase/execution_state:unknown`，不伪装成功。`scripts/agent/probe_multi_aitable_dry_run.py --output docs/agent-scans/multi-aitable-script-contract-20260809.md` 实测 **9/9 PASS**，覆盖可发现性、异常兜底、当前信封成功回包透传与导入触发不确定态，且不保存 JSON fixture。`bulk_add_fields.py`、`import_records.py`、`upload_attachment.py` 本轮仅证明 legacy dry-run 零写入，未宣称相同异常/终态契约。 | 按实际 Skill 路由逐个迁移其余高风险脚本；先做受控 child-runner 与真实终态验证，再扩展共享运行时，禁止把“有 dry-run”扩大成“写入终态已验证” |
