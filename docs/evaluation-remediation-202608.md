...
| Agoal 整域 Runtime Schema exclusion 与 Skill 正向路由矛盾 | **首个只读叶已完成逐命令准入与统一投影；其余 15 条继续审阅** | Mono/Multi Skill 已长期正向教授 `dws agoal`，但 Runtime Schema 曾把 16 条业务叶整体排除，导致 Skill 路由与 Schema 发现面相互矛盾。本轮没有为追求 exclusion=0 批量放开：仅将无必填参数、可安全读取当前用户规则周期的 `agoal user rules` 补齐 ProductDecl、ContractFinal、read/low/not_required/idempotent Safety、`user-id/request-id` 参数映射和 canonical Agent 示例，从 exclusion 精确移除。随后按独立提交完成 `legacy_only → dual_validate → unified_active`：同一次 `get_user_rules` 调用严格校验顶层、规则、周期、引用关系、整数精度和重复稳定 ID；未知字段、非空但未审阅的绩效任务/规则内容及矛盾引用均返回不可重试的 `projection_unknown`，不静默丢数据。普通 `--format json` 现直接返回 `ok/outcome/data/meta`，规则与周期句柄固定为 `ruleId/periodId`，`meta.count` 对齐；服务端没有分页/覆盖证据，因此明确 `ruleCoverageKnown:false` 且不生成 `meta.pagination`。真实当前用户只读返回 1 条稳定规则，dual 阶段 legacy 字节保持、active 阶段统一投影通过，stderr 均为空；扫描不保存用户、规则、周期或原始 JSON，见 `agent-scans/agoal-user-rules-surface-20260810.md`、`agent-scans/agoal-user-rules-dual-validation-20260810.md`、`agent-scans/agoal-user-rules-unified-projection-20260810.md`。当前全量 Schema 仍只有 1 条 Agoal public tool，其余 15 条保持排除 | 继续按叶审阅 11 条读取和 4 条覆盖式写入；读命令先补稳定结果投影/分页边界，写命令必须补 confirmation、dry-run、旧数据覆盖风险、partial/unknown 与回读证据后才可移出 exclusion。合法 CLI 生命周期、重复 canonical 与敏感数据边界不要求清零 |
