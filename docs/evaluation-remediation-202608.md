...
| Todo 聚合 shortcut 将未知响应伪装为空集合 | **代码已修，待真实环境复验** | `+related-tasks`、`+due-today`、`+created-todos`、`+overdue` 与写前定位的 `+todo-done` 复用严格 `todoCards` 投影：仅明确数组可继续；缺容器、容器非数组、非法行或无稳定 `taskId` 均在本地 fail-closed 为不可重试的 `api/projection_unknown` / `response_projection`。这会在按标题完成待办前阻断异常读取，而不是误报“没找到”或继续发写请求。Agent 审阅证据：`docs/agent-scans/todo-aggregate-projection-20260809.md` | 用隔离账号复验正常空列表、多页、权限受限和未知形状；`+todo-done` 再验证投影失败时绝不发写请求 |
...
| Chat 已加入群列表未知响应伪装为空结果 | **CLI 已 fail-closed；主入口完成真实单页投影复验** | `chat +chat-list-all` 与 Agent 主入口 `chat +my-groups` 现在仅对明确识别的群数组返回空列表。未知容器、非法群条目或无稳定 `conversationId` 返回 typed `projection_unknown`；自动翻页中后续页异常则保留已读群及 `failures` 台账，标记 `stopReason=projection_error` 且不安全重试，避免将部分群目录当完整结果。当前源码构建二进制对 `+my-groups --limit 1` 的真实只读扫描返回 1 条投影群、`count` 对齐、稳定 `conversationId` 与可判定的续页事实；该命令仍为 legacy JSON，不能误报 unified active，见 `agent-scans/chat-my-groups-live-projection-20260809.md`。 | 用真实账号验证空群、跨页、游标错误和网关响应形状漂移；真实群目录覆盖范围仍不由 CLI 擅自承诺，`+my-groups` 继续按 legacy→dual→active 逐步迁移 |
...
| AITable/Base 目录死条目与假阴 | **CLI 已 fail-closed，服务端未关闭** | 列表/搜索结果显式标记发现来源、分页已知性和索引覆盖未知，不再承诺权威全量目录；`query_records` 完整结果使用 `endpointExhausted:true`，不再输出语义过宽的 `complete` 键；`base list/search` 会聚合顶层与 `result/data` 的分页证据：`hasMore=true` 无游标、`hasMore=false` 带游标、字段非布尔、多个 continuation 不一致或外层/嵌套 `hasMore` 矛盾都会返回不可重试的 `pagination_inconsistent`，一致的嵌套证据才会投影分页事实，不再产出不可续翻的成功结果。当前真实最近访问读取保留 `authoritativeInventory:false` / `paginationKnown:false`，一条投影记录的 `baseId` 与 `count` 对齐，见 `agent-scans/aitable-base-discovery-live-20260809.md`；它不能证明目录精度或全量覆盖。 | 后端治理死条目；真实账号复测精度和召回率 |
...
| `sheet +list-sheets` 未知响应可能伪装为空列表 | **CLI 已 fail-closed；统一输出 dual 验证中** | `get_all_sheets` 投影现在区分“已知空列表”和“未知响应”；未知容器、非法行或只有展示标题而无稳定 `sheetId` 时返回 typed `projection_unknown`，不会向 Agent 产出成功的空或不可继续操作的工作表列表。统一 `CommandResult` 已带 `meta.count`，但作为既有命令当前仍先处于 `dual_validate`：外部 `--format json` 保持 legacy payload，shadow result 已验证后才可由 Agent 审阅晋级。未凭空声明分页终态。回归覆盖空列表、嵌套容器、非法响应、display-only 条目与 dual 阶段 legacy wire | 发布后二进制用真实表格验证正常空表格与后端响应漂移两条路径；通过命令级 Agent 审阅后再晋级 unified active |
