...
| 错误 subtype / 恢复提示缺少闭集治理 | **十批 registry + 文档/event/IM/统一输出/复合写/插件协议/身份参数 family 已落地，机制未关闭** | `scripts/agent/scan_error_contract.py` 从非测试 Go 源码确认 **74** 个 descriptor、**130** 个直接 `WithSubtype` 调用和 **11** 个经测试的有限间接映射；高频本地校验、输入/公式/下载完整性、自然目标解析、AITable 目标预检、版本预检、transport/服务端响应、本地 flag/Skill 市场、文档复合写、`event_stop_unverified`、IM 八条幂等只读 `*_incomplete` 分页 family、unified output 的 `pagination_*` / `invalid_success_type` / `skill_setup_*`、三条复合写部分失败、stdio/MCP/plugin/format 的八条协议失败，以及 Agent code/host/product 三条本地身份参数 family 均已进入稳定 registry。第十批保留 legacy `reason` wire；初始化/发现失败声明为 `idempotent_read_only`，`tools/call`、显式 MCP error 与空响应默认不可盲重放并要求先核查写终态；身份参数只做本地 validation，不泄露不可信环境变量原文。未知 HTTP/RPC/响应体和网络观测继续归 category 对应的 `upstream_unclassified` / `discovery_upstream_unclassified`，保留 `http_status`、`rpc_code`、trace、`transport_failure` 等诊断事实但不把上游数字或网络文本拼成 Agent subtype；401/403、限流、协议不兼容和后端依赖不可用也有独立恢复边界。`tools/call` 的模糊 408/5xx/网络失败仍不宣称安全重试。doc create/checkpoint-update/history-revert 与 event stop 已在 dual validation 构建严格 outcome result，同时保留 legacy error/rc，尚未转 active。其余仍有 **31** 个自由字面 `WithReason` 调用、9 个直接 `ErrorInfo.Subtype`（均已登记）与 1 个动态构造点；扫描到的 subtype **0** 个缺有效恢复提示。enum 转 string 写法由 Agent 扫描纳入统计。扫描只产出 Markdown 评测证据，不保存 JSON fixture，也不把 Agent 审阅替换成 CI。 | 按 [RFC-0003](rfcs/0003-error-contract-subtype-governance.md) 逐命令迁移剩余自由 subtype，补齐 registry 所需恢复语义；剩余个人订阅状态机必须连同 Category、幂等性与终态语义单独设计，不能仅作字符串替换；文档/event 的 typed result bridge 按 [RFC-0005](rfcs/0005-operation-outcome-bridge.md) 逐命令完成 legacy golden 与受控真实账号验证后再转 active，再逐步要求每个公开 subtype 的 recovery 语义 |
| IM 分页不完整错误仍是自由字符串 | **第六批 registry 已收口读路径** | 群搜索/已加入群/收藏/会话/单聊消息/全量消息/我的群/话题回复的 8 条 `*_incomplete` reason 已改为稳定 subtype；保持既有 reason wire，不改变 legacy 消费方。每条声明为 API + `idempotent_read_only`，命令级 hint 继续要求保留已读结果与 `failures`，按 `nextCursor`/`nextPage` 续读，不能把当前结果当完整。`go test ./internal/errors ./internal/shortcut/chat ./internal/shortcut/smart` 通过 | 继续将其余 35 个自由 reason 按读写安全性、幂等性与终态证据逐条审阅；不能把写请求的模糊失败套用此 read-only 策略 |
| 统一输出直接 subtype 未登记/缺恢复提示 | **第七批已关闭直接分支键缺口** | DevApp 分页 `pagination_invalid` / `pagination_incomplete` / `pagination_conflict`、shortcut 非布尔 `success`、以及 Skill 安装全失败/非法 partial 均改用登记 enum，并输出 hint；DevApp 分页错误还带 `operation=devapp.pagination_projection`，shortcut 类型漂移带 `operation=shortcut.response_projection`，Skill 安装全失败带 `operation=skill.setup`。现有结果类型、reason、退出码不变；Agent 扫描确认 9 个直接 `ErrorInfo.Subtype` 均已登记 | 持续审阅其余 46 个 legacy `WithReason` 和 16 条缺有效恢复提示的 family；不得把“已登记”误写成真实服务端终态已验证 |
