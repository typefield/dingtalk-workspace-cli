...
| Todo 我的待办未知响应伪装为空结果 | **CLI 已 fail-closed，统一输出已 active** | `todo +get-my-tasks` 现在仅在 `result.todoCards` 被明确识别为空数组时返回成功空列表；未知容器、非法行、仅展示字段或无稳定 `taskId` 返回 typed `projection_unknown`，不再把上游响应漂移说成“当前没有待办”。普通 `--format json` 直接输出统一结果；页码输入不等于 endpoint 终态，续页事实未确认时显式标 `data.pagination_known:false` | 用真实组织验证空列表、正常任务、分页和服务端异常形状；写入终态问题仍按 `+add-participant` 的真实环境验收执行 |
