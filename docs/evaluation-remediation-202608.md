...
| 12 条读命令 12 种 JSON 信封 | **框架已接入，渐进迁移中** | Framework 2.0 已提供统一 `ok/outcome/data/error/meta`、强类型布尔、单 writer、partial/pending；不公开协议选择参数，不输出版本标记。`drive download` 已作为文件传输首条迁入：成功、dry-run、缺参失败均由同一根出口生成统一返回 | legacy 命令按 terminal command 继续迁移；不得把迁移责任交给 Agent |
...
| Mail 公开读取列表未知响应被伪装为成功空结果 | **CLI 已 fail-closed** | `mail +thread-list`、`+folder-list`、`+tag-list`、`+user-search`、`+template-list`、`+contact-list` 现区分已知空数组与未知响应；未知容器、非法行或无可识别字段返回 typed `projection_unknown`，不再向 Agent 声称“没有会话/文件夹/标签/用户/模板/联系人” | 用真实邮箱分别复验正常空结果、搜索无命中、分页读取和服务端异常形状；分页证据完整后再逐命令纳入统一结果 |
...
| `drive download --format json` 惰性、stdout 日志污染 | **代码已修，已迁入统一返回** | 下载成功与 dry-run JSON 路径均有测试；该 terminal command 现由 Framework writer 单次输出：成功/preview 为 stdout 的统一返回，失败同样为可机读统一 failure；进度只写 stderr。安全 CLI 实跑已确认 dry-run 输出顶层 `ok/outcome/dry_run/data`，缺 `--node` 输出 typed validation/rc=3，均不含版本字段 | 发布后二进制执行真实下载成功与上游失败两条 `jq` 管道复验 |
