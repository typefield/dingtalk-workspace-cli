...
| 错误 subtype / 恢复提示缺少闭集治理 | **Agent 台账已建立，机制未关闭** | `scripts/agent/scan_error_contract.py` 从非测试 Go 源码识别 79 个字面 `WithReason` subtype、6 个直接 `ErrorInfo.Subtype` 与 16 个动态构造点；当前 `Category`、退出码、`hint/actions/retryable/retry_after_seconds` 已可复用，但 `WithReason(string)` 仍是自由字符串，不能宣称 `type/subtype` 已全量 wire-stable。扫描只产出 Markdown 评测证据，不保存 JSON fixture，也不把 Agent 审阅替换成 CI。RFC-0003 已明确渐进边界：先治理高频 subtype，不改公开命令/输出选择方式。 | 按 [RFC-0003](rfcs/0003-error-contract-subtype-governance.md) 审定高频跨产品 subtype（`missing_required_flags`、`unknown_flag`、`confirmation_required`、`rate_limit`、`pagination_inconsistent`、`projection_unknown`）并建立 descriptor 注册表；动态上游 reason 必须显式映射或归入 `api/upstream_unclassified`，再逐步要求每个公开 subtype 的 recovery 语义 |
