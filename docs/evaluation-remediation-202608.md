...
| `dev connect --daemon` 启动/重启过早报告 success | **未关闭（生命周期语义）** | 代码在 `child.Start()`/`Process.Release()` 后立即返回 `status:started`；supervisor 随后才写 `daemon-state.json`，worker heartbeat 还要更晚产生。`restart` 同样只等待 re-exec 返回并 best-effort 回读 state，未验证 supervisor/worker 已就绪 | 增加 bounded readiness handshake：至少确认匹配 supervisor state 且进程存活；若在预算内没有 heartbeat/就绪状态，返回 pending 或 typed failure，不得返回终态 success。为 start/restart 增加真实 detached 子进程回归，覆盖启动失败、worker 立即退出和成功握手 |
