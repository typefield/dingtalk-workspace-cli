...
| 错误 subtype / 恢复提示缺少闭集治理 | **首批 registry 已落地，机制未关闭** | `scripts/agent/scan_error_contract.py` 从非测试 Go 源码确认首批 6 个 descriptor，已有 10 个 `WithSubtype` 调用（含 HTTP 429 → `rate_limit`）；其余仍有 148 个自由字面 `WithReason` 调用、6 个直接 `ErrorInfo.Subtype` 与 16 个动态构造点。`Category`、退出码、`hint/actions/retryable/retry_after_seconds` 已可复用，但未注册值仍不能宣称 wire-stable。扫描只产出 Markdown 评测证据，不保存 JSON fixture，也不把 Agent 审阅替换成 CI。RFC-0003 已明确渐进边界：先治理高频 subtype，不改公开命令/输出选择方式。 | 按 [RFC-0003](rfcs/0003-error-contract-subtype-governance.md) 逐命令迁移高频 subtype 的剩余自由调用，补齐 registry 所需恢复语义；动态上游 reason 必须显式映射或归入 `api/upstream_unclassified`，再逐步要求每个公开 subtype 的 recovery 语义 |
...
| IM 分页算法与统一返回尚未合流 | **框架基元已落地，首批四条 IM 命令双验证** | `internal/output.PageLedger` 已实现有限页预算、游标推进/循环校验、耗尽/续页/未知三态、分页 meta 生成，以及首屏失败→`failure`、后续页失败/未知→`partial_failure` 的统一结果映射。只读 `chat +flag-list`、`chat +chat-search`、`chat +conversation-list` 与 `chat +thread-replies` 已进入 `dual_validate`：每条命令只执行一次业务流程，同时构建 shadow PageLedger/CommandResult，legacy stdout 与错误行为保持不变；分页矛盾在 shadow 中 fail closed。`chat-search` 的同 cursor最大窗口探测只计一个逻辑页；`conversation-list` 严格区分已知空数组与未知投影，并把本地页上限表达为可续成功；`thread-replies` 已把时间游标、重复游标、未知回复容器和后续页失败映射到同一账本，但带 `--download-resources` 的本地写入路径仍仅双验证。其余消息仍多以各 shortcut 自定义的 legacy 分页 payload 输出。中央 `server_failure_classifier` 也尚未获得调用的读写/幂等/执行状态。 | 对拍首批四条命令的 legacy bytes 与 shadow/临时 active outcome 后再启用统一返回；下一批迁消息列表。写调用先接入安全上下文，再治理 retryable |
...
| event 停机契约 | **安全入口与部分失败表达已修，真实停机待复验** | `event stop` 为 destructive/high/user_required；无 `--yes` 拦截，`--dry-run` 不停订阅；缺 `subscribe_id/--all` 或两者冲突均在本地返回 typed validation、rc=3，不再误报 internal、rc=5。本地消费者停止失败时保留 run state；远端订阅已部分取消但后续取消/本地清理失败时，返回结构化 `partial_failure`（保留 succeeded/failed IDs、unknown execution state、不可盲重试提示） | 真实订阅执行 stop 后验证进程、订阅和本地状态三者终态一致 |
