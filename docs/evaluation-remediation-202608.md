...
| Chat 已加入群列表未知响应伪装为空结果 | **CLI 已 fail-closed；主入口已进入 dual_validate，并完成真实单页投影复验** | `chat +chat-list-all` 与 Agent 主入口 `chat +my-groups` 现在仅对明确识别的群数组返回空列表。未知容器、非法群条目或无稳定 `conversationId` 返回 typed `projection_unknown`；自动翻页中后续页异常则保留已读群及 `failures` 台账，标记 `stopReason=projection_error` 且不安全重试，避免将部分群目录当完整结果。当前源码构建二进制对 `+my-groups --limit 1` 的真实只读扫描返回 1 条投影群、`count` 对齐、稳定 `conversationId` 与可判定的续页事实，见 `agent-scans/chat-my-groups-live-projection-20260809.md`。本轮命令从 legacy 进入 dual_validate：外部仍保持历史 JSON，内部由 `PageLedger` 候选结果表达端点耗尽、可续页、未知边界及后续页 `partial_failure`；不公开版本或 rollout 参数。 | 用真实账号验证空群、跨页、游标错误和网关响应形状漂移；真实群目录覆盖范围仍不由 CLI 擅自承诺，完成脱敏 live evidence 后才按单命令评审进入 active |
