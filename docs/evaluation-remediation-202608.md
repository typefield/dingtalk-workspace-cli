...
| 错误 subtype / 恢复提示缺少闭集治理 | **三批 registry 已落地，机制未关闭** | `scripts/agent/scan_error_contract.py` 从非测试 Go 源码确认 30 个 descriptor、104 个 `WithSubtype` 调用；高频本地校验、输入/公式/下载完整性、自然目标解析、AITable 目标预检与文档/表格版本预检均已无自由构造调用。第三批将 `version_not_found`、目标缺失/歧义/类型冲突、候选不完整、未知上游目标响应、回读未验证和附件 token 缺失收进稳定 registry：Category、只读重试边界和默认恢复提示同源，且保持既有 `Reason` wire。真实 CLI 已验证 `invalid_json_input` 返回 `validation/invalid_json_input/rc=3` 和修复 hint；目标/版本路径由单元回归保持既有 reason。其余仍有 54 个自由字面 `WithReason` 调用、6 个直接 `ErrorInfo.Subtype` 与 16 个动态构造点，16 个 subtype 缺有效恢复提示。扫描只产出 Markdown 评测证据，不保存 JSON fixture，也不把 Agent 审阅替换成 CI。RFC-0003 已明确渐进边界：先治理高频 subtype，不改公开命令/输出选择方式。 | 按 [RFC-0003](rfcs/0003-error-contract-subtype-governance.md) 逐命令迁移剩余自由 subtype，补齐 registry 所需恢复语义；动态上游 reason 必须显式映射或归入 `api/upstream_unclassified`，再逐步要求每个公开 subtype 的 recovery 语义 |
