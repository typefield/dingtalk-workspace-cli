...
| Multi 脚本 Help/写入预览与结果漂移 | **受控结果边界已覆盖 AITable、Todo、OA 与 Mail 的高风险入口；真实终态待验证** | 当前文件/入口统计以最新 Agent 全量审计报告为准。AITable import/field/record/attachment 四个明确写入口支持脚本级 `--dry-run`，受控 fixture 不调用 dws/OSS；Todo 和 OA 分别保留逐项结果与审批任务选择事实。AITable 维持可独立安装的局部 `_runtime.py`；Mail 因已声明依赖 `dingtalk-shared`，迁入 shared runtime，由它统一 `--format text|json|ndjson`、`--dry-run`、异常兜底、child DWS 明确失败/终态未知分类和单一 JSON stdout。`mail_send_with_cc.py` 现在要求 `--yes`，dry-run 不读取邮箱或发送；旧业务布尔 `success:false` 返回 typed failure，发送后必须取得 `internetMessageId` 并以 `mail message verify` 的 `sendStatus=success` 才标为 `verification.state=verified`。投递中、无标识或查询失败都保留未知/非终态，不建议重发；服务端仅报告 `partial_success` 却不给逐收件人明细时，输出 `partial_unknown` failure 而不伪造 `succeeded[]/failed[]`。`scripts/agent/probe_multi_mail_send_contract.py --output docs/agent-scans/multi-mail-send-contract-20260809.md` 为 8/8 Agent 语义证据，只保存 Markdown，不保存 JSON fixture。 | 按实际 Skill 路由逐个迁移其余高风险脚本；先做受控 child-runner 与真实终态验证，再扩展共享运行时，禁止把“有 dry-run”扩大成“写入终态已验证” |
...
| Multi 局部结果边界之间的实现漂移 | **Agent 语义对拍已建立；仍为渐进迁移** | AITable、Todo、Misc 继续保留可独立安装的本地 `_runtime.py`。Mail 已声明依赖 `dingtalk-shared`，因此显式使用其中的 shared runtime，而不是隐藏地依赖另一个产品 Skill；shared runtime 同样对严格布尔旧结果、异常边界、stdout 与 0/1/7 退出码负责。`scripts/agent/probe_multi_runtime_boundaries.py` 横向验证本地运行时，`scripts/agent/probe_multi_mail_send_contract.py` 验证 shared Mail 路径；二者仅输出 Markdown Agent 证据，不保存 JSON fixture。 | 随下一批迁移继续执行 Agent 对拍；共享运行时只允许被 Skill 明确声明的依赖使用，避免脆弱的跨产品相对导入 |
...
- Multi 的 Agent 扫描见 `docs/agent-scans/skill-contract-audit-20260809.md`：
  ...
  提及 `--dry-run`、7/42 提及脚本级 `--format`（这不是 argparse 能力证明）。这不是要求强行给所有工具增加两个参数，
