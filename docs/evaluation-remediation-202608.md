...
| Agoal 整域 Runtime Schema exclusion 与 Skill 正向路由矛盾 | **首个只读叶已完成逐命令准入；其余 15 条继续审阅** | Mono/Multi Skill 已长期正向教授 `dws agoal`，但 Runtime Schema 曾把 16 条业务叶整体排除，导致 Skill 路由与 Schema 发现面相互矛盾。本轮没有为追求 exclusion=0 批量放开：仅将无必填参数、可安全读取当前用户规则周期的 `agoal user rules` 补齐 ProductDecl、ContractFinal、read/low/not_required/idempotent Safety、`user-id/request-id` 参数映射和 canonical Agent 示例，从 exclusion 精确移除。当前全量 Schema 只有 1 条 Agoal public tool，其余 15 条仍保持排除。源码临时构建后的 Help/Schema 发现通过；真实当前用户只读返回 1 条带稳定规则 ID 的记录，stderr 为空，扫描不保存用户、规则、周期或原始 JSON，见 `agent-scans/agoal-user-rules-surface-20260810.md`。当前输出仍是 legacy JSON，本项只关闭发现面 exclusion，不伪称统一输出已完成 | 继续按叶审阅 11 条读取和 4 条覆盖式写入；读命令先补稳定结果投影/分页边界，写命令必须补 confirmation、dry-run、旧数据覆盖风险、partial/unknown 与回读证据后才可移出 exclusion。合法 CLI 生命周期、重复 canonical 与敏感数据边界不要求清零 |
