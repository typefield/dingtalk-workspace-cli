...
| contact 投影层 `8/5/1` 数据丢失 | **代码已修，待真实环境复验** | `+list-roles` 已拍平分组 `labels[]`；角色/部门成员识别 `labelUserList`/`deptUserList` 并解包 `userInfo`。回归测试锁定报告中的 8/5/1 下层计数；任一非空条目无法投影时整体 fail-closed，不再返回成功空/残缺列表。三条 8/5/1 读取已迁入统一结果；缺稳定 `labelId` 或 `userId` 的展示行会 fail-closed，仍保留数值型 label ID。另 `+list-followings`、姓名/手机号搜索、`+list-sub-depts` 也已进入统一结果：缺明确列表容器、非法行、缺稳定 `userId/openDingTalkId` 或 `deptId` 均返回 typed `projection_unknown`；特别关注保留服务端真实的 `openDingTalkId`，部门数值 ID 也保持原类型，不强制转换为展示字符串 | 用原评测账号复跑三条 8/5/1 命令及四条发现命令，分别对拍 lower/upper 数量、稳定 ID 和统一输出；不把本地 projector fixture 视作真实组织目录覆盖证明 |
...
- **Mono 复合写确认门禁（2026-08-09）**：此前 9 个复合远端写入口只依赖 Skill 的文字约束，脚本自身缺少 `--yes`，可被直接调用绕过。现已将确认门禁收进 `_runtime.py`，并接入文档创建写入、邮件发送、日程创建、批量待办、OA 批量审批、字段创建、记录导入、AITable 文件导入和附件上传。`mono-write-confirmation-20260809.md` 以临时 HOME/工作区和 sentinel `dws` 对 9/9 入口省略 `--yes` 的非 dry-run 调用验证：均返回 `policy/confirmation_required`、`execution_state=not_executed`，未观察到 child CLI 调用或新增本地文件。`--dry-run` 仍可免确认生成计划；确认后的真实租户终态和 exactly-once 仍为 UNVERIFIED。
