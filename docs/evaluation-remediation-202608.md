...
| IM 分页算法与统一返回尚未合流 | **框架基元已落地；`+flag-list` 已 active，余四条双验证** | `internal/output.PageLedger` 已实现有限页预算、游标推进/循环校验、耗尽/续页/未知三态、分页 meta 生成，以及首屏失败→`failure`、后续页失败/未知→`partial_failure` 的统一结果映射。只读 `chat +flag-list` 现为 `unified_active`：普通 `--format json` 直接生成单一 `ok/outcome/data/error/meta` 结果，不公开版本/协议选择参数；本地页上限保留 `endpoint_exhausted:false + next_token`，未知边界省略分页断言，后续页失败保留成功页并返回 `partial_failure`/rc=7。Agent 扫描 [chat-flag-list-contract-20260809](agent-scans/chat-flag-list-contract-20260809.md) 为 5/5 PASS；它只验证本地 fixture 契约，不替代真实账号。`chat +chat-search`、`chat +conversation-list`、`chat +thread-replies` 与 `chat +chat-messages` 仍为 `dual_validate`：每条仅执行一次业务流程，同时构建 shadow PageLedger/CommandResult，legacy stdout 与错误行为保持不变。`chat-search` 的同 cursor最大窗口探测只计一个逻辑页；`conversation-list` 严格区分已知空数组与未知投影；`thread-replies` 与 `chat-messages` 已把毫秒时间游标、重复游标、未知容器和后续页失败映射到同一账本。后二者的 `--download-resources`、以及 `chat-messages --output` 本地写入路径仍仅双验证。其余消息仍多以各 shortcut 自定义的 legacy 分页 payload 输出。中央 `server_failure_classifier` 也尚未获得调用的读写/幂等/执行状态。 | 以真实账号复验 `+flag-list` 的收藏召回、响应形状和索引健康；完成 legacy bytes、shadow 与 active outcome 对拍后，逐条迁移余四个只读入口。写调用先接入安全上下文，再治理 retryable |
