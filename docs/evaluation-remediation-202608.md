...
| Mono 脚本 Help/Skill 统计漂移 | **本轮已关闭（Help 层）** | 35 个 Python 文件按 AST 分为 32 个 Agent 入口、3 个内部模块；Agent 扫描实测 32/32 暴露 `--dry-run`、32/32 暴露 `--format`、Help 非零为 0；`attendance_report_checkin.py` 的可选依赖检查已延后到业务执行阶段 | 受控探针当前覆盖 7 个高风险写入口并全部通过；其余入口的业务参数组合、异常路径和真实账号副作用仍需按台账标记为 `UNVERIFIED` |
