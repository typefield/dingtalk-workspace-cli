...
| Mono 脚本 Help/Skill 统计与错误路径漂移 | **Help/机器错误边界与四条写编排已收口；真实终态仍待验证** | 35 个 Python 文件按 AST 分为 32 个 Agent 入口、3 个内部模块；Agent 扫描实测 32/32 暴露 `--dry-run`、32/32 暴露 `--format`、Help 非零为 0；`attendance_report_checkin.py` 的可选依赖检查已延后到业务执行阶段；全部入口现经 `_runtime.run_main` 进入统一边界：JSON/NDJSON 下未捕获异常返回 `failure + error.type=internal`，不以 traceback 替代结果。运行时还验证缓存后的机器 stdout：JSON 必须是一条 `ok/outcome` 信封且与 0/1/7 退出码一致，NDJSON 每个非空行必须是对象；遗留 `print()`、多行、非法 JSON 或结果/退出码矛盾均被拒绝并转为单一 typed failure，原始污染文本不会回显给 Agent。共享 `run_child_dws` 对统一 `ok:false` 与旧信封布尔 `success:false` 都保守视为非成功，字符串 `"false"` 不按 truthiness 推断；不可证实的写入终态仍标为 `unknown`。`todo_batch_create.py` 的错误类型 `executors` 直接返回 validation；待办、审批、文档与记录导入批处理均使用 `succeeded/failed/unknown` 三通道，审批任务解析失败不会发送占位 task ID，文档 update 不会自动重放；`mail_send_with_cc.py` 已接入同一边界，旧业务失败不会被误报为已发送。Skill 已说明 `partial_failure`/rc=7、三通道和可选 `meta` 语义 | `scripts/agent/probe_mono_result_contract.py --output docs/agent-scans/mono-result-contract-20260809.md` 实测 **15/15**：32/32 入口接入、未捕获异常、stdout 污染拒绝、结果/退出码一致性、子 dws 严格布尔失败识别、非零 SystemExit、partial rc=7、meta、错误类型输入、邮件旧业务失败和四条受控 child-runner 编排均为单一 JSON stdout；`scripts/agent/probe_mono_dry_run.py --output docs/agent-scans/mono-dry-run-probe-20260809.md` 对 8 个高风险写入口实测 8/8：单一 JSON stdout、无 sentinel dws 调用、无额外本地文件；其余入口的业务参数组合、其他脚本的子 dws meta、异常路径和真实账号副作用仍需标记为 `UNVERIFIED` |
| Mono 邮件/日程/导入写入结果真实性 | **代码已收口，真实终态待验证** | `mail_send_with_cc.py` 不再把旧 `success:false` 业务失败报告为“已发送”；`calendar_schedule_meeting.py` 在事件已创建、参会人或订房后续失败/不确定时保留 `succeeded[]`，并将后续步骤放入 `failed[]/unknown[]`，输出 `partial_failure`、rc=7；缺少 eventId 同样不伪装成功。`import_records.py` 也按批次保存确认成功、明确失败和终态未知，后续批次不再抹掉前序成功。`mono-result-contract-20260809.md` 现为 **15/15 PASS**，覆盖旧业务失败、日程与导入后续写入失败及共享信封边界 | 用隔离账号验证邮件、事件、参会人、会议室及导入记录的真实最终状态；受控 child-runner 不等同于服务端 exactly-once 证据 |
...
- **2026-08-09 复扫**：`mono-script-contract-20260809.md` 重新确认 35 个 Python 文件中 32 个为 Agent 入口，Help 为 `32/32` dry-run、`32/32` format、0 个非零；`mono-result-contract-20260809.md` 对共享 `run_main` 的未捕获异常、stdout 污染拒绝、结果/退出码一致性、非零 `SystemExit`、partial rc=7、meta 透传、严格布尔子失败识别、邮件旧业务失败和四条 child-dws 编排路径均为 `15/15 PASS`。因此“类型错误导致 traceback 打破 JSON”“脚本信封不能承载 meta”以及“旧 `success:false` 被当作邮件已发送”都是修复前快照的问题；记录导入也不再把已确认批次压成总布尔失败。`mono-dry-run-probe-20260809.md` 用临时 HOME/工作区和 sentinel dws 验证 8 个深层门控写入口均未调用子进程、未新增本地文件；它仍不替代真实后端零写或其余 24 个入口的逐路径验证。
