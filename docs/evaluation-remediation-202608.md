...
| Multi 局部结果边界之间的实现漂移 | **Agent 语义对拍已建立；仍为渐进迁移** | AITable（含异步 export）、Todo、Misc 继续保留可独立安装的局部 `_runtime.py`。Mail、Doc 和 Calendar 均明确要求加载 `dingtalk-shared`，因此显式使用其中的 shared runtime，而不是隐藏地依赖另一个产品 Skill；shared runtime 同样对严格布尔/非布尔旧结果、异常边界、三通道、stdout 与 0/1/7 退出码负责。`scripts/agent/probe_multi_runtime_boundaries.py` 横向验证本地与 shared 运行时，Mail/Doc/Calendar/AITable-export 专项探针验证产品语义；它们仅输出 Markdown Agent 证据，不保存 JSON fixture。 | 随下一批迁移继续执行 Agent 对拍；共享运行时只允许被 Skill 明确要求加载的依赖使用，避免脆弱的跨产品相对导入 |
...
- Multi 的 Agent 扫描见 `docs/agent-scans/skill-contract-audit-20260809.md`：
  ...
  提及 `--dry-run`、10/42 提及脚本级 `--format`（这不是 argparse 能力证明）。这不是要求强行给所有工具增加两个参数，
