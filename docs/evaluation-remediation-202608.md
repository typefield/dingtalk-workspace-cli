...
| Mono 脚本 Help/Skill 统计与错误路径漂移 | **Help/机器错误边界与关键写编排已收口；真实终态仍待验证** | 35 个 Python 文件按 AST 分为 32 个 Agent 入口、3 个内部模块；Agent 扫描实测 32/32 暴露 `--dry-run`、32/32 暴露 `--format`、Help 非零为 0；`attendance_report_checkin.py` 的可选依赖检查已延后到业务执行阶段；全部入口现经 `_runtime.run_main` 进入统一边界：JSON/NDJSON 下未捕获异常返回 `failure + error.type=internal`，不以 traceback 替代结果。运行时还验证缓存后的机器 stdout：JSON 必须是一条 `ok/outcome` 信封且与 0/1/7 退出码一致，NDJSON 每个非空行必须是对象；遗留 `print()`、多行、非法 JSON 或结果/退出码矛盾均被拒绝并转为单一 typed failure，原始污染文本不会回显给 Agent。共享 `run_child_dws` 对统一 `ok:false` 与旧信封布尔 `success:false` 都保守视为非成功，字符串 `"false"` 不按 truthiness 推断；不可证实的写入终态仍标为 `unknown`。`todo_batch_create.py` 的错误类型 `executors` 直接返回 validation；待办、审批、文档、记录导入、字段创建、文件导入与附件上传均保留明确成功、执行前失败和终态未知，审批任务解析失败不会发送占位 task ID，文档 update 不会自动重放；`mail_send_with_cc.py` 已接入同一边界，旧业务失败不会被误报为已发送。Skill 已说明 `partial_failure`/rc=7、三通道和可选 `meta` 语义 | `scripts/agent/probe_mono_result_contract.py --output docs/agent-scans/mono-result-contract-20260809.md` 实测 **18/18**：32/32 入口接入、未捕获异常、stdout 污染拒绝、结果/退出码一致性、子 dws 严格布尔失败识别、非零 SystemExit、partial rc=7、meta、错误类型输入、邮件旧业务失败与关键受控写入编排均为单一 JSON stdout；`scripts/agent/probe_mono_dry_run.py --output docs/agent-scans/mono-dry-run-probe-20260809.md` 对 8 个高风险写入口实测 8/8：单一 JSON stdout、无 sentinel dws 调用、无额外本地文件；其余入口的业务参数组合、其他脚本的子 dws meta、异常路径和真实账号副作用仍需标记为 `UNVERIFIED` |
| Mono 邮件/日程/表格写入结果真实性 | **代码已收口，真实终态待验证** | `mail_send_with_cc.py` 不再把旧 `success:false` 业务失败报告为“已发送”；`calendar_schedule_meeting.py` 在事件已创建、参会人或订房后续失败/不确定时保留 `succeeded[]`，并将后续步骤放入 `failed[]/unknown[]`，输出 `partial_failure`、rc=7；缺少 eventId 同样不伪装成功。`import_records.py` 按批次保存确认成功、明确失败和终态未知，后续批次不再抹掉前序成功；`bulk_add_fields.py` 不再把旧业务 `success:false` 报告为“字段创建完成”；`aitable_import_via_task.py` 在上传成功后若 `import_data` 未给出终态，会保留 `importId`、阶段与 `execution_state:unknown`，不建议盲目重复触发；`upload_attachment.py` 则在 PUT 未得终态时保留 `fileToken`、上传阶段和未知状态，不误报附件可用。`mono-result-contract-20260809.md` 现为 **18/18 PASS**，覆盖旧业务失败、日程与表格写入后续失败及共享信封边界 | 用隔离账号验证邮件、事件、参会人、会议室、导入记录、字段创建、文件导入与附件上传的真实最终状态；受控 child-runner 不等同于服务端 exactly-once 证据 |
| Multi 脚本 Help/写入预览与结果漂移 | **受控结果边界已覆盖 4 个 AITable 写入口；真实终态待验证** | 55 个 Python 文件、42 个 Agent 入口；逐个 Agent 执行 `--help` 为 42/42。AITable import/field/record/attachment 四个明确写入口支持脚本级 `--dry-run`，受控 fixture 不调用 dws/OSS。新建表文件导入 `aitable_import_via_task.py`、字段批量创建 `bulk_add_fields.py`、附件上传 `upload_attachment.py` 与记录批量导入 `import_records.py` 已迁入 Multi 自有 `_runtime.py`：`--format text|json|ndjson`、`--dry-run`、单一 JSON 结果、异常兜底与 child DWS 的明确失败/终态未知分类均由同一边界处理；导入触发不可信时保留 `importId/phase/execution_state:unknown`，字段创建与附件 PUT 未确认时保留上下文和 `execution_state:unknown`，但不宣称已创建或令牌可用；记录批次保留 `succeeded[]/failed[]/unknown[]`，后续批失败不会抹掉前序成功。`scripts/agent/probe_multi_aitable_dry_run.py --output docs/agent-scans/multi-aitable-script-contract-20260809.md` 为 Agent 语义探针，不保存 JSON fixture；它覆盖可发现性、异常兜底、当前信封成功回包透传、文件/字段/附件不确定态及批量导入部分成功。 | 按实际 Skill 路由逐个迁移其余高风险脚本；先做受控 child-runner 与真实终态验证，再扩展共享运行时，禁止把“有 dry-run”扩大成“写入终态已验证” |
...
| Multi 局部结果边界之间的实现漂移 | **Agent 语义对拍已建立；仍为渐进迁移** | AITable、Todo、Misc 各自保留可独立安装的本地 `_runtime.py`，避免把 Skill 的机器输出隐式依赖另一个产品 Skill。为防止三份边界在异常、历史布尔值或部分成功语义上悄然分叉，`scripts/agent/probe_multi_runtime_boundaries.py` 会在临时 Python 子进程中横向加载三份运行时，验证未捕获异常只输出 typed internal failure、旧 `success:false` 不会当成成功、`partial_failure` 保留三通道且退出码为 7；输出仅为 Markdown Agent 证据，不保存 JSON fixture。 | 随下一批迁移继续执行该 Agent 对拍；长期再评估受控共享运行时/打包策略，不能用脆弱的跨 Skill 相对导入破坏独立安装 |
...
- **2026-08-09 复扫**：`mono-script-contract-20260809.md` 重新确认 35 个 Python 文件中 32 个为 Agent 入口，Help 为 `32/32` dry-run、`32/32` format、0 个非零；`mono-result-contract-20260809.md` 对共享 `run_main` 的未捕获异常、stdout 污染拒绝、结果/退出码一致性、非零 `SystemExit`、partial rc=7、meta 透传、严格布尔子失败识别、邮件旧业务失败和关键 child-dws 编排路径均为 `18/18 PASS`。因此“类型错误导致 traceback 打破 JSON”“脚本信封不能承载 meta”以及“旧 `success:false` 被当作邮件已发送”都是修复前快照的问题；记录导入不再把已确认批次压成总布尔失败，字段创建不再将旧业务失败误报为成功，文件导入在 `import_data` 未给终态时会保留任务标识，附件上传在 PUT 未得终态时也不再误报为可用。`mono-dry-run-probe-20260809.md` 用临时 HOME/工作区和 sentinel dws 验证 8 个深层门控写入口均未调用子进程、未新增本地文件；它仍不替代真实后端零写或其余 24 个入口的逐路径验证。
...
- Multi 的 Agent 扫描见 `docs/agent-scans/multi-script-contract-20260808.md`：
  55 个 Python 文件、42 个入口，当前 42/42 `--help` 成功；Help 文本已有 30/42
