...
| 错误 subtype / 恢复提示缺少闭集治理 | **十四批 registry 已消除字面自由 reason；动态个人订阅状态机仍待审阅** | `scripts/agent/scan_error_contract.py` 从非测试 Go 源码确认 **102** 个 descriptor、**163** 个直接 `WithSubtype(...)` 或兼容桥调用和 **11** 个经测试的有限间接映射；自由字面 `WithReason` 已从 31 降为 **0**，9 个直接 `ErrorInfo.Subtype` 均已登记，所有 stable subtype 均有有效恢复提示。第十四批收口个人订阅静态 guard/validation、legacy `partial_failure`、显式业务失败和 toolbar `SYSTEM_BUSY`：后者不再伪装 validation，而是 API 写入不确定态（`execution_started:true`、不可盲重放，先核查当前快捷入口状态）。`request_build_failed` 使用兼容桥保留 legacy `reason`，并按 API/discovery 投影不同稳定 subtype；既有 `reason` 消费方不受影响。唯一剩余动态构造是个人订阅状态机，仍须连同 Category、幂等性和真实终态单独审阅。未知 HTTP/RPC/响应体和网络观测继续归 unclassified subtype，`tools/call` 的模糊 408/5xx/网络失败仍不宣称安全重试。扫描只产出 Markdown 评测证据，不保存 JSON fixture，也不把 Agent 审阅替换成 CI。 | 继续用受控账号审阅个人订阅的动态 failure family；文档/event 的 typed result bridge 按 [RFC-0005](rfcs/0005-operation-outcome-bridge.md) 完成 legacy golden 与真实账号验证后再转 active；不得把 registry 或本地单测误写成服务端终态已经验证 |
| IM 分页不完整错误仍是自由字符串 | **第六批 registry 已收口读路径** | 群搜索/已加入群/收藏/会话/单聊消息/全量消息/我的群/话题回复的 8 条 `*_incomplete` reason 已改为稳定 subtype；保持既有 reason wire，不改变 legacy 消费方。每条声明为 API + `idempotent_read_only`，命令级 hint 继续要求保留已读结果与 `failures`，按 `nextCursor`/`nextPage` 续读，不能把当前结果当完整。`go test ./internal/errors ./internal/shortcut/chat ./internal/shortcut/smart` 通过 | 字面自由 reason 已清零；剩余动态个人订阅状态机必须按读写安全性、幂等性与终态证据单独审阅，不能把写请求的模糊失败套用此 read-only 策略 |
| 统一输出直接 subtype 未登记/缺恢复提示 | **第七批已关闭直接分支键缺口** | DevApp 分页 `pagination_invalid` / `pagination_incomplete` / `pagination_conflict`、shortcut 非布尔 `success`、以及 Skill 安装全失败/非法 partial 均改用登记 enum，并输出 hint；DevApp 分页错误还带 `operation=devapp.pagination_projection`，shortcut 类型漂移带 `operation=shortcut.response_projection`，Skill 安装全失败带 `operation=skill.setup`。现有结果类型、reason、退出码不变；Agent 扫描确认 9 个直接 `ErrorInfo.Subtype` 均已登记 | 字面 legacy `WithReason` 已为 **0**，只剩 1 个动态状态机构造；不得把“已登记”误写成真实服务端终态已验证 |
