...
| metadata-only 命令的本地参数错误被归类为 internal | **框架已修** | `LeafSpec.Validate` 是写入前的本地输入校验钩子；普通 error 现由框架统一转为 typed validation、rc=3，主动返回的 API/auth 等 typed error 保持原类别。`event stop` 缺 `subscribe_id/--all`、目标冲突、无 `--yes` 与有效 dry-run 已以 CLI 实跑验证 | 继续将旧命令的 RunE 内部参数检查迁到声明层；不把远端响应或本地 I/O 故障误包为 validation |
