...
| Mail 公开读取列表未知响应被伪装为成功空结果 | **CLI 已 fail-closed；4 条分页读命令已迁入统一返回** | `mail +thread-list`、`+folder-list`、`+tag-list`、`+user-search`、`+template-list`、`+contact-list` 现区分已知空数组与未知响应；未知容器、非法行或无可识别字段返回 typed `projection_unknown`，不再向 Agent 声称“没有会话/文件夹/标签/用户/模板/联系人”。`+thread-list`、`+user-search`、`+template-list`、`+contact-list` 还严格投影其公开的 `hasMore/nextCursor`：有续页必须带 token、终态不得带普通 token、外层/嵌套矛盾返回 `pagination_inconsistent`；随后才迁入统一返回，`meta.count` 与 `meta.pagination` 不再被丢弃 | 用真实邮箱复验四条命令的正常空结果、续页、终态与异常形状；`+folder-list`/`+tag-list` 未声明分页证据，保持旧返回直至有独立契约 |
