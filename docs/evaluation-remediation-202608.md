...
| Mono 脚本 Help/Skill 统计与错误路径漂移 | **Help/机器错误边界与关键写编排已收口；只读聚合继续逐脚本治理** | 35 个 Python 文件按 AST 分为 32 个 Agent 入口、3 个内部模块；Agent 扫描实测 32/32 暴露 `--dry-run` 与 `--format`，Help 非零为 0。所有入口均经 `_runtime.run_main` 统一机器异常边界：未捕获异常、stdout 污染、非法 JSON、无 typed error、meta/dry_run 类型错误或结果/退出码矛盾，都会改写为单一 typed failure；旧 `success:false`、非布尔/矛盾状态和子 pending 都不会冒充终态成功。Skill 已说明 `partial_failure`/rc=7、三通道和 meta。Todo 的日期汇总/逾期检查不再用手写 subprocess 抹掉 child 结果：首屏 typed failure 原样分类，后续页失败保留已读页并返回 `partial_failure`/rc=7，达到 10 页硬上限时以 `unknown page:11` 拒绝伪称完整，page meta 透传到外层。 | `probe_mono_result_contract.py` 为 **24/24 PASS**，覆盖 32/32 入口异常边界、严格 child 结果解析与代表性写编排；`probe_mono_dry_run.py` 为 **10/10 PASS**；`probe_mono_todo_read_contract.py` 为 **12/12 PASS**，以临时 child runner 对拍 Todo 两入口的短页、后续页失败、首页失败、硬上限、text rc 与 dry-run。以上均不替代真实账号副作用、端点耗尽和服务端终态证据；其余仍手写 child subprocess 的只读脚本须按实际路由继续迁移，不能把共享 `meta` 参数存在扩大成全量已透传。 |
