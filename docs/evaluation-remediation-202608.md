...
| 错误 subtype / 恢复提示缺少闭集治理 | **四批 registry 已落地，机制未关闭** | `scripts/agent/scan_error_contract.py` 从非测试 Go 源码确认 37 个 descriptor、105 个直接 `WithSubtype` 调用和 6 个经测试的有限间接映射；高频本地校验、输入/公式/下载完整性、自然目标解析、AITable 目标预检、版本预检与 transport/服务端响应均已进入稳定 registry。第四批将未知 HTTP/RPC/响应体失败收敛为 category 对应的 `upstream_unclassified` / `discovery_upstream_unclassified`，保留 `http_status`、`rpc_code`、trace 等诊断事实但不再把上游数字拼成 Agent subtype；401/403、限流、协议不兼容和后端依赖不可用也有独立恢复边界。`tools/call` 的模糊 408/5xx/网络失败仍不宣称安全重试。其余仍有 54 个自由字面 `WithReason` 调用、6 个直接 `ErrorInfo.Subtype` 与 7 个动态构造点，16 个 subtype 缺有效恢复提示。扫描只产出 Markdown 评测证据，不保存 JSON fixture，也不把 Agent 审阅替换成 CI。 | 按 [RFC-0003](rfcs/0003-error-contract-subtype-governance.md) 逐命令迁移剩余自由 subtype，补齐 registry 所需恢复语义；剩余动态 reason 必须显式映射到当前 Category 对应的 unclassified subtype 或经 Agent 审阅后注册，再逐步要求每个公开 subtype 的 recovery 语义 |
