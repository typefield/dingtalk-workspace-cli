...
| devapp 核心写 shortcut 将 API success 扩大为业务终态 | **四条命令已逐条迁入统一结果；删除保持 dual** | `devapp +create/+update/+enable/+disable` 与对应 `dev app` 原子入口共用 `DevAppMutationResultSpec` 和成功投影。普通 `--format json` 的成功 data 显式带 `verification.state=not_verified`，说明当前没有写后回读；非对象响应 fail-closed 为不可重试的 `api/projection_unknown`，并标记 `execution_started:true`，提示先查询目标状态、禁止盲目重试。`+update` 零变更字段在业务调用前返回 typed validation；四条命令的 dry-run、确认门禁、一次调用、统一 success 与未知投影由 [Agent 扫描](agent-scans/devapp-core-write-outcomes-20260810.md) 验证为 6/6 PASS | 用隔离应用完成真实创建、修改、启停后的回读终态验证；`devapp +delete` 在补齐原子入口已有的 `--confirm-name` 二次防误删前继续保持 `dual_validate`，不得为追求迁移数字提前 active |
...
3. 继续复验 DevApp 权限受限、其他账号和异常响应；对四条 active 核心写命令完成真实
   写后回读，确认 `verification:not_verified` 不被消费方扩大成终态成功。为
   `devapp +delete` 补齐 `--confirm-name` 二次防误删后再审阅晋级。终页位置 cursor 不得
