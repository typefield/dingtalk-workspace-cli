...
| `sheet +list-sheets` 未知响应可能伪装为空列表 | **CLI 已 fail-closed，统一输出已 active** | `get_all_sheets` 投影现在区分“已知空列表”和“未知响应”；未知容器、非法行或只有展示标题而无稳定 `sheetId` 时返回 typed `projection_unknown`，不会向 Agent 产出成功的空或不可继续操作的工作表列表。普通 `--format json` 直接输出统一结果与 `meta.count`；未凭空声明分页终态。回归覆盖空列表、嵌套容器、非法响应和 display-only 条目 | 发布后二进制用真实表格验证正常空表格与后端响应漂移两条路径 |
