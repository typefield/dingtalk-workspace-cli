...
| Minutes 听记列表未知响应伪装为空结果 | **CLI 已 fail-closed** | `minutes +list-mine`、`+list-shared`、`+list-all` 共享投影器以及 `+minutes-search` 现在区分已知空列表和未知响应；未知容器、非法条目或没有稳定 `taskUuid` 的条目返回 typed `projection_unknown`，不再把服务端形状漂移扩大成“没有听记”。搜索、`+latest-minutes`、`+action-items` 与 `+transcript` 现共用同一 `list_by_keyword_and_time_range` 容器识别，包含服务端常见 `result.itemList`；后面三条复合读只有明确空数组才会提示“暂无妙记”，未知容器、非法项或无稳定 taskUuid 均 fail-closed。搜索结果额外明确 `index_coverage_known:false` / `pagination_known:false`，空命中不扩张成索引健康或业务不存在。`+detail` 也已迁入统一结果：多 artifact 读取全成功才返回 success；部分失败保留成功 artifact 与 typed failed 条目并以 `partial_failure`/rc=7 返回，全失败才是普通 API failure | 用真实账号分别验证我创建、他人共享、关键词无命中、`result.itemList` 正常形状及分页 token；对 `+detail` 用真实任务验证基础信息/摘要/关键词/逐字稿/待办的单项失败与全部失败；当前仍不把无完整分页或索引证据的命令承诺为完整终态 |
