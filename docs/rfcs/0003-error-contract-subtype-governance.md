...
| 事实 | 数量 | 含义 |
|---|---:|---|
| 已注册 descriptor / 直接 `WithSubtype(Subtype...)` 调用 / 间接映射 | 45 / 107 / 11 | 首批八个、输入/公式/下载完整性第二批五个、目标解析/版本预检第三批十七个、transport/服务端响应第四批七个、本地 flag/Skill 市场第五批三个，以及文档复合写固定五个 reason family 已落地；间接映射函数或有限局部状态选择由单元测试证明只返回有限注册值；迁移保持既有 `Reason` 字符串 wire，不引入版本标记 |
