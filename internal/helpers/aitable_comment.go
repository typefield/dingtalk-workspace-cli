...
endpoint_exhausted=false 时必须保持 baseId、tableId、recordId 不变，并把 meta.pagination.next_token 原样传给下一次 --cursor。`,
		...
		Contract: LeafContract{
			...
			Result: &contract.ResultSpec{
				...
				DataSchema: aitableResultSchemaWithDryRun("AI 表格记录评论列表或 dry-run 请求预览", aitableCommentListResultSchema),
			},
...
func aitableCommentItemResultSpec() *contract.ResultSpec {
	return &contract.ResultSpec{
		Outcomes:   []contract.ResultOutcome{contract.ResultOutcomeSuccess, contract.ResultOutcomeFailure},
		DataSchema: aitableResultSchemaWithDryRun("AI 表格记录评论业务结果或 dry-run 请求预览", aitableCommentItemResultSchema),
	}
}
