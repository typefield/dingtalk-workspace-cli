...
func (p *OAuthProvider) refreshWithRefreshTokenForProfile(ctx context.Context, data *TokenData, profile string) (*TokenData, error) {
	if data.Source == "mcp" {
		return p.refreshViaMCPForProfile(ctx, data, profile)
	}
	clientID := data.ClientID
	if clientID == "" {
		clientID = ClientID()
	...
	body := map[string]string{
		...
		"refreshToken": data.RefreshToken, "grantType": "refresh_token",
	}
	response, err := p.postJSON(ctx, UserAccessTokenURL, body)
	if err != nil {
		return nil, err
	}
	updated, err := p.parseTokenResponse(response)
	if err != nil {
		return nil, err
	}
	preserveRefreshedIdentity(updated, data)
	if err := saveTokenDataLockedForProfile(p.configDir, updated, profile); err != nil {
		return nil, fmt.Errorf("保存刷新后的 token 失败（旧 refresh_token 已失效，请重新登录）: %w", err)
	}
	return updated, nil
}
...
func (p *OAuthProvider) refreshViaMCPForProfile(ctx context.Context, data *TokenData, profile string) (*TokenData, error) {
	clientID := data.ClientID
	if clientID == "" {
		clientID = ClientID()
	}
	...
	body := map[string]string{
		"clientId": clientID, "refreshToken": data.RefreshToken, "grantType": "refresh_token",
	}
	response, err := p.postJSON(ctx, GetMCPBaseURL()+MCPRefreshTokenPath, body)
	if err != nil {
		return nil, err
	}
	updated, err := p.parseMCPTokenResponse(response)
	if err != nil {
		return nil, err
	}
	preserveRefreshedIdentity(updated, data)
...
func preserveRefreshedIdentity(updated, original *TokenData) {
	updated.ClientID = original.ClientID
	updated.Source = original.Source
	updated.PersistentCode = original.PersistentCode
	updated.CorpID = original.CorpID
	updated.UserID = original.UserID
	updated.UserName = original.UserName
	if updated.CorpName == "" {
		updated.CorpName = original.CorpName
	}
}
