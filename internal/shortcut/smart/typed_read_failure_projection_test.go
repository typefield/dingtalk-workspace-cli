...
func TestCompositeReadFailuresPreserveTypedRecoveryFacts(t *testing.T) {
	...
	cases := map[string]func(error) *output.ErrorInfo{
		"thread replies": threadRepliesReadFailureInfo,
		"my groups":      myGroupsReadFailureInfo,
		"at me":          atMeReadFailureInfo,
		"search message": searchMsgReadFailureInfo,
		"todo aggregate": todoCardsReadFailureInfo,
	}
	for name, project := range cases {
		t.Run(name, func(t *testing.T) {
			info := project(typed)
			assertCompositeTypedReadFailure(t, info)
		})
	}
}

func assertCompositeTypedReadFailure(t *testing.T, info *output.ErrorInfo) {
	t.Helper()
	if info == nil {
