...
func TestOrgUnifiedResultUsesReviewedProjection(t *testing.T) {
	if Org.OutputRollout != output.RolloutUnifiedActive {
	...
	if !ok || data["deptId"] != float64(7) || data["deptName"] != "Engineering" || data["memberCount"] != float64(3) || len(data) != 3 {
