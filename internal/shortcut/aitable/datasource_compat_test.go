...
func TestCrossPlatformCoverageDatasourceUpdateCompatibleSourceConfig(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			caller := &datasourceCoverageCaller{respond: func(gotCtx context.Context, tool string) (string, error) {
			...
			argv := append([]string{"+datasource-update", "--base-id", "B", "--table-id", "T"}, tc.flags...)
			if err := runDatasourceShortcutCLIContext(t, ctx, caller, argv...); err != nil {
				t.Fatal(err)
			}
			...
			if len(caller.toolLog) != len(wantTools) {
				t.Fatalf("calls = %v", caller.toolLog)
			}
			for i, wantTool := range wantTools {
				if caller.toolLog[i] != wantTool || caller.serverLog[i] != "aitable" {
					t.Fatalf("wrong target: %v", caller.toolLog)
				}
				...
				if !reflect.DeepEqual(caller.argLog[i], want) {
					t.Fatalf("%s args = %#v, want %#v", wantTool, caller.argLog[i], want)
				}
			}
		})
	}
...
func TestCrossPlatformCoverageDatasourceUpdateReadFailureNeverWrites(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			caller := &datasourceCoverageCaller{respond: func(context.Context, string) (string, error) { return tc.response, tc.readErr }}
			err := runDatasourceShortcutCLIContext(t, context.Background(), caller, "+datasource-update", "--base-id", "B", "--table-id", "T", "--auto")
			if err == nil || len(caller.toolLog) != 1 {
				t.Fatalf("err=%v calls=%v", err, caller.toolLog)
			}
			i := 0
			if caller.toolLog[i] != "get_datasource_config" {
				t.Fatalf("unexpected write: %v", caller.toolLog)
			}
		})
	}
}
...
func TestCrossPlatformCoverageDatasourceUpdateValidatesBeforeRead(t *testing.T) {
	for _, flags := range [][]string{
		...
		argv := append([]string{"+datasource-update", "--base-id", "B", "--table-id", "T"}, flags...)
		if err := runDatasourceShortcutCLIContext(t, context.Background(), caller, argv...); err == nil || len(caller.toolLog) != 0 {
			t.Fatalf("flags=%v err=%v calls=%v", flags, err, caller.toolLog)
		}
	}
}
...
func TestCrossPlatformCoverageDatasourceUpdateCancellationAndWriteFailure(t *testing.T) {
	for _, cancelAfterRead := range []bool{true, false} {
		...
		t.Cleanup(cancel)
		caller := &datasourceCoverageCaller{respond: func(_ context.Context, stringTool string) (string, error) {
		...
		err := runDatasourceShortcutCLIContext(t, ctx, caller, "+datasource-update", "--base-id", "B", "--table-id", "T", "--auto")
		wantCalls := 2
		if cancelAfterRead {
			wantCalls = 1
		}
		if err == nil || len(caller.toolLog) != wantCalls {
			t.Fatalf("cancel=%v err=%v calls=%v", cancelAfterRead, err, caller.toolLog)
		}
...
func TestCrossPlatformCoverageDatasourceUpdateDryRunNeverWrites(t *testing.T) {
	caller := &datasourceCoverageCaller{dryRun: true, respond: func(context.Context, string) (string, error) {
		return `{"status":"success","data":{"datasourceType":"OA","sourceConfig":"{\"processCode\":\"P\"}"}}`, nil
	}}
	argv := []string{"+datasource-update", "--base-id", "B", "--table-id", "T", "--auto"}
	argv = append(argv, "--dry-run")
	_ = runDatasourceShortcutCLIContext(t, context.Background(), caller, argv...)
	for i := range caller.toolLog {
		if caller.toolLog[i] != "get_datasource_config" {
			t.Fatalf("dry run wrote: %v", caller.toolLog)
		}
	}
}
