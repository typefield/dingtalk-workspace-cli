		...
		if format == "jsonml" && content != "" {
			if nodeID == "" {
				return docPartialWriteError(
					rt, "doc.create", apperrors.SubtypeDocCreateMissingNodeID, "resolve_created_document",
			...
			if _, err := rt.CallMCPWriteData(productDoc, "update_document", map[string]any{"nodeId": nodeID, "format": "jsonml", "jsonml": content, "mode": "overwrite"}); err != nil {
				return docPartialWriteError(
					rt, "doc.create", apperrors.SubtypeDocCreateInitialContentFailed, "write_jsonml",
...
var CheckpointUpdate = shortcut.Shortcut{
	...
	Execute: func(rt *shortcut.RuntimeContext) error {
		...
		if _, err := rt.CallMCPWriteData(productDoc, "update_document", map[string]any{"nodeId": rt.Str("node"), "markdown": content, "mode": rt.Str("mode")}); err != nil {
			return checkpointPartialWriteError(rt, rt.Str("node"), checkpoint, "update", apperrors.SubtypeDocCheckpointUpdateFailed, err,
		...
		if err != nil {
			return checkpointPartialWriteError(rt, rt.Str("node"), checkpoint, "verify", apperrors.SubtypeDocCheckpointVerificationFailed, err,
...
func checkpointPartialWriteError(rt *shortcut.RuntimeContext, nodeID string, checkpoint map[string]any, stage string, subtype apperrors.Subtype, cause error, steps []map[string]any) error {
	...
	return docPartialWriteError(
		rt, "doc.checkpoint_update", subtype, stage,
