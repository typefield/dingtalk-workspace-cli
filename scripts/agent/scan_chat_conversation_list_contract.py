#!/usr/bin/env python3
"""Agent semantic scan for the active chat +conversation-list pagination contract.

This is an evidence collector, not a CI gate. It verifies the public command
declaration and fixture-backed promotion tests. It never calls a DingTalk
...
DECLARATION = ROOT / "internal" / "shortcut" / "chat" / "chat_conversation.go"
TEST = ROOT / "internal" / "shortcut" / "chat" / "chat_conversation_projection_test.go"
...
def source_checks() -> list[tuple[str, str, str]]:
    ...
    match = re.search(
        r"var ConversationList = shortcut\.Shortcut\{(?P<body>.*?)^\}",
        declaration,
    ...
    return [
        (
            ...
            "ConversationList 必须直接使用 unified_active；Agent 不选择协议版本。",
        ),
        (
            "终页零游标不被误报为矛盾",
            "PASS" if "rawCursorErr != nil || rawNextCursor != 0" in declaration else "FAIL",
            "hasMore=false + nextCursor=0 是 API 的终止哨兵，必须输出 endpoint_exhausted=true。",
        ),
        (
            "分页账本决定统一结果",
            "PASS" if "pageLedger.Result(unifiedData)" in declaration and "rt.OutputResult(payload, result)" in declaration else "FAIL",
            "业务读取一次后由 PageLedger 生成 CommandResult，再走单一输出出口。",
        ),
        (
            "Agent 调用形式覆盖 --format json",
            "PASS" if '"--format", "json"' in test else "FAIL",
            "promotion probe 只使用公开 format 参数，不存在协议选择 flag。",
        ),
        (
            "不暴露版本选择标记",
...
def run_probe() -> tuple[str, str]:
    ...
    command = [
        ...
        "-run", r"TestConversationList(PaginationRolloutIsUnifiedActive|UnifiedPromotionEvidence)$",
    ]
...
def main() -> int:
    ...
    lines += [
        "",
        f"结论：**{passed}/{len(checks)} PASS**。`chat +conversation-list --format json` 已使用统一结果：`hasMore=false + nextCursor=0` 正确声明 endpoint 已耗尽；本地页上限返回可恢复 token；未知边界不声明耗尽；后续页读取失败保留成功页并返回 `partial_failure`/rc=7。",
        "",
