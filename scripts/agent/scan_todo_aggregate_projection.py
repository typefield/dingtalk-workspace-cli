...
PATTERN = r"TestShortcutTodoCards|TestCreatedTodosRejectsUnknownTodoProjection|TestTodoAggregateShortcutsHaveReadOnlyContracts|TestTodoAggregateRolloutStartsDualValidate|TestRelatedTasksDualValidatePreservesLegacyBytes|TestRelatedTasksDualValidatePreservesLegacyFailureWithoutPayload|TestRelatedTasksUnifiedResultPreservesEarlierPagesOnLaterFailure|TestTodoAggregateLegacyFacadeRemainsFailClosedForWritePreflight"
...
def main() -> int:
    ...
    missing_dual_routes = [
        route for route, path in DUAL_READ_ROUTES.items()
    ...
    passed = result.returncode == 0 and shared_ok and partial_reader_ok and not missing_dual_routes and not missing_legacy_routes and write_fail_closed

    transcript = (result.stdout + result.stderr).strip()
    if len(transcript) > 3000:
        transcript = transcript[-3000:]
    missing_dual = ", ".join(f"`{route}`" for route in missing_dual_routes) or "无"
...
- 已复核 dual 读取入口：**{len(DUAL_READ_ROUTES) - len(missing_dual_routes)}/{len(DUAL_READ_ROUTES)}**
- 未使用保留页读取器的 dual 入口：{missing_dual}
...
5. `+related-tasks`、`+due-today` 在 `dual_validate` 阶段必须保持 legacy stdout；未来激活后，后续页失败须以 `partial_failure`/rc=7 保留已读页。
