...
# runtime payload and Linux ABI build helpers. Other pkg/ and scripts/ packages belong to the
...
list_coverage_scope() {
  cd "$ROOT"
  go list ./ ./cmd/... ./internal/... ./skills/... ./scripts/build/runtime-payload ./scripts/build/linux-abi
}
